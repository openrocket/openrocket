...
public class CGCaretGenerator {

	private static final float RADIUS = 0.06f;
	private static final float BORDER_THICKNESS_RATIO = 0.1f; // 10% of radius
	...
	public static Mesh create(GraphicsQualitySettings.RenderQuality renderQuality) {
		...
		generatePieSlice(vertices, indices, RADIUS, segments, (float) Math.PI);
		generateQuarterAnnulus(vertices, indices, RADIUS, BORDER_THICKNESS_RATIO, segments, (float) Math.PI);

		// 2. Top-right quarter
		generatePieSlice(vertices, indices, RADIUS, segments, 0.0f);
		generateQuarterAnnulus(vertices, indices, RADIUS, BORDER_THICKNESS_RATIO, segments, 0.0f);

		// 3. Center ring
		generateRing(vertices, indices, RADIUS, RADIUS * 0.9f, segments * 2);
	...
	private static void generatePieSlice(List<Vertex> vertices, IntList indices, float radius, int segments, float startAngle) {
		...
		for (int i = 0; i <= segments; i++) {
			float angle = startAngle + (float) i / segments * (float) Math.PI / 2f;
	...
	private static void generateQuarterAnnulus(List<Vertex> vertices, IntList indices, float radius, float borderThicknessRatio, int segments, float startAngle) {
		...
		for (int i = 0; i <= segments; i++) {
			...
			float yOuter = (float) Math.sin(angle) * radius;
