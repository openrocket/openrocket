...
public class OpenRocketAPI {
	...
	// private FlightDataBranch m_CFlightDataBranch = null; //use
	// GetFlightData()
	// private FlightDataBranch psas_fdb =null;
	...
	/**
	 * Returns one value correlating to the key type and the current iteration.
	 * 
	 * @param Specify
	 *            this type of data to return
	 * @return double
	 */
	 ...
	 * 
	 * @param Desired
	 *            iteration (the first iteration is iteration 1)
	 * @return FlightDataStep type.
	 */
	 ...
	 * @throws Throwable
	 */
	protected FlightDataBranch GetFlightData() throws IllegalStateException {
	...
	public boolean IsSimulationStagesRunning() {
		return m_bIsSimulationStagesRunning;
	}

	public boolean IsSimulationLoopRunning() {
		return m_bIsSimulationLoopRunning;
	}
	...
	public int StartSimulation() {
		...
		FlightData fm_temp = new FlightData(new FlightDataBranch("empty",
				FlightDataType.TYPE_TIME));
		try {
			m_CStatus = m_CRocket.firstInitialize(m_CSimulationConditions,
					m_CStatus, fm_temp);
			if (m_CStatus == null) {
	 ...
	 * 					-1 (simcount<1) simulation in file 
	 *                  -2 !(simcount < simtograb) simulation specified not present
	 * 					-3 simulation data not present in simulation
	 * 					-4 exception thrown
	 * 
	 * @param  string	filename to load
	 * @param  int		simulation in file to use
	 */
	...
	public int LoadRocket(String szFileName, int simtograb) {
		try {
			...
			if (!(simCount < simtograb)) {
				...
				if (temp2 != null) {
					...
					System.out.println("status of rocket is "
							+ temp2.getStatus());
					m_CSimulationConditions = temp2.getOptions()
			...
			// TODO: return loadorkfile(szFileName); //this needs to be more
			// complex...
			// TODO: what does complex mean?
	...
	public int RunSimulation() {
		if (m_bIsSimulationStagesRunning == true) {
			System.err
					.println("error calling RunSimulation while StartSimulation is running may Invalidate StartSimulations FlightData");
			return -1;
		}
		if (m_CSimulationConditions == null) {
	 ...
	 *****************************************************/
	/*
	 * Were never going to return any values. RK4 ignores the flightdata object.
	 * public double getMaxAltitude() { if (m_CFlightData == null) return -1;
	 * return m_CFlightData.getMaxAltitude(); } public double getMaxVelocity() {
	 * if (m_CFlightData == null) return -1; return
	 * m_CFlightData.getMaxVelocity(); } public double getMaxAcceleration() { if
	 * (m_CFlightData == null) return -1; return
	 * m_CFlightData.getTimeToApogee(); } public double getMaxMachNumber() { if
	 * (m_CFlightData == null) return -1; return
	 * m_CFlightData.getMaxMachNumber(); } public double getTimeToApogee() { if
	 * (m_CFlightData == null) return -1; return
	 * m_CFlightData.getTimeToApogee(); } public double getFlightTime() { if
	 * (m_CFlightData == null) return -1; return m_CFlightData.getFlightTime();
	 * } public double getGroundHitVelocity() { if (m_CFlightData == null)
	 * return -1; return m_CFlightData.getGroundHitVelocity(); } public double
	 * getLaunchRodVelocity() { if (m_CFlightData == null) return -1; return
	 * m_CFlightData.getLaunchRodVelocity(); } public double
	 * getDeploymentVelocity() { if (m_CFlightData == null) return -1; return
	 * m_CFlightData.getDeploymentVelocity(); }
	 */
