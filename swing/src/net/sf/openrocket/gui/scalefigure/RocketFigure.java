...
/**
 * A <code>ScaleFigure</code> that draws a complete rocket. Extra information
 * can be added to the figure by the methods
 * {@link #addRelativeExtra(FigureElement)}, {@link #clearRelativeExtra()}.
 * 
...
public class RocketFigure extends AbstractScaleFigure {
	...
	/*
	 * figureComponents contains the corresponding RocketComponents of the
	 * figureShapes
	 */
	private final ArrayList<Shape> figureShapes = new ArrayList<Shape>();
	private final ArrayList<RocketComponent> figureComponents = new ArrayList<RocketComponent>();
	...
	/**
	 * Set the configuration displayed by the figure. It may use the same or
	 * different rocket.
	 * 
	 * @param configuration
	 ...
	 * <p>
	 * Warning: If paintComponent is used outside the normal Swing usage, some
	 * Swing dependent parameters may be left wrong (mainly transformation). If
	 * it is used, the RocketFigure should be repainted immediately afterwards.
	 */
	@Override
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < figureShapes.size(); i++) {
			...
			if (color == null) {
				color = Application.getPreferences().getDefaultColor(
						c.getClass());
			}
			...
			if (style == null)
				style = Application.getPreferences().getDefaultLineStyle(
						c.getClass());
			...
			if (selected) {
				g2.setStroke(new BasicStroke((float) (SELECTED_WIDTH
						* EXTRA_SCALE / scale), BasicStroke.CAP_BUTT,
						BasicStroke.JOIN_BEVEL, 0, dashes, 0));
				g2.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL,
						RenderingHints.VALUE_STROKE_PURE);
			} else {
				g2.setStroke(new BasicStroke((float) (NORMAL_WIDTH
						* EXTRA_SCALE / scale), BasicStroke.CAP_BUTT,
						BasicStroke.JOIN_BEVEL, 0, dashes, 0));
				g2.setRenderingHint(RenderingHints.KEY_STROKE_CONTROL,
						RenderingHints.VALUE_STROKE_NORMALIZE);
		...
		g2.setStroke(new BasicStroke(
				(float) (NORMAL_WIDTH * EXTRA_SCALE / scale),
		...
		Color fillColor = ((SwingPreferences) Application.getPreferences())
				.getMotorFillColor();
		Color borderColor = ((SwingPreferences) Application.getPreferences())
				.getMotorBorderColor();
			...
			for (Coordinate coord : position) {
				...
				if (type == TYPE_SIDE) {
					s = new Rectangle2D.Double(EXTRA_SCALE * coord.x,
							EXTRA_SCALE * (coord.y - radius), EXTRA_SCALE
									* length, EXTRA_SCALE * 2 * radius);
				} else {
					s = new Ellipse2D.Double(EXTRA_SCALE * (coord.z - radius),
							EXTRA_SCALE * (coord.y - radius), EXTRA_SCALE * 2
									* radius, EXTRA_SCALE * 2 * radius);
				}
	...
	private Shape[] getShapes(RocketComponent component) {
		...
		switch (type) {
		case TYPE_SIDE:
			m = Reflection.findMethod(ROCKET_FIGURE_PACKAGE, component,
					ROCKET_FIGURE_SUFFIX, "getShapesSide",
					RocketComponent.class, Transformation.class);
			break;

		case TYPE_BACK:
			m = Reflection.findMethod(ROCKET_FIGURE_PACKAGE, component,
					ROCKET_FIGURE_SUFFIX, "getShapesBack",
					RocketComponent.class, Transformation.class);
			break;
		...
		if (m == null) {
			Application.getExceptionHandler().handleErrorCondition(
					"ERROR: Rocket figure paint method not found for "
							+ component);
			return new Shape[0];
		}
	...
	/**
	 * Gets the bounds of the figure, i.e. the maximum extents in the selected
	 * dimensions. The bounds are stored in the variables minX, maxX and maxR.
	 */
