...
public class SimulationRunDialog extends JDialog {
	private static final Logger log = LoggerFactory
			.getLogger(SimulationRunDialog.class);
	...
	static {
		...
		executor = new ThreadPoolExecutor(n, n, 0L, TimeUnit.MILLISECONDS,
	...
	public SimulationRunDialog(Window window, OpenRocketDocument document,
			Simulation... simulations) {
		// // Running simulations...
		super(window, trans.get("SimuRunDlg.title.RunSim"),
				Dialog.ModalityType.APPLICATION_MODAL);
		this.document = document;

		if (simulations.length == 0) {
			throw new IllegalArgumentException(
					"Called with no simulations to run");
		}
		...
		for (int i = 0; i < n; i++) {
			...
			simulationWorkers[i] = new InteractiveSimulationWorker(document,
					simulations[i], i);
		...
		// // Simulation time:
		panel.add(new JLabel(trans.get("SimuRunDlg.lbl.Simutime") + " "),
				"gapright para");
	 ...
	 */
	public static void runSimulations(Window parent,
			OpenRocketDocument document, Simulation... simulations) {
	...
	private void updateProgress() {
		...
		timeLabel.setText(u.toStringUnit(simulationStatuses[index]
				.getSimulationTime()));

		u = UnitGroup.UNITS_DISTANCE.getDefaultUnit();
		altLabel.setText(u.toStringUnit(simulationStatuses[index]
				.getRocketPosition().z)
				+ " (max. "
	...
	private class InteractiveSimulationWorker extends SimulationWorker {
		...
		public InteractiveSimulationWorker(OpenRocketDocument doc,
				Simulation sim, int index) {
			super(sim);
			...
			while (iterator.hasNext()) {
				...
				if (m.getIgnitionConfiguration().getDefault()
						.getIgnitionEvent() == IgnitionConfiguration.IgnitionEvent.LAUNCH)
					launchBurn = MathUtil.max(launchBurn, m
							.getMotorConfiguration().get(id).getMotor()
							.getBurnTimeEstimate());
				else
					otherBurn = otherBurn
							+ m.getMotorConfiguration().get(id).getMotor()
									.getBurnTimeEstimate();
			}
		...
		protected void process(List<SimulationStatus> chunks) {
			...
			for (SimulationStatus s : chunks) {
				simulationMaxAltitude[index] = Math.max(
						simulationMaxAltitude[index], s.getRocketPosition().z);
				simulationMaxVelocity[index] = Math.max(
						simulationMaxVelocity[index], s.getRocketVelocity()
								.length());
			}
			...
			if (simulationStage == -2
					&& status.getSimulationTime() < burnoutTimeEstimate) {
				log.debug("Method 1:  t=" + status.getSimulationTime()
						+ "  est=" + burnoutTimeEstimate);
				setSimulationProgress(MathUtil.map(status.getSimulationTime(),
						0, burnoutTimeEstimate, 0.0, BURNOUT_PROGRESS));
				updateProgress();
				return;
			}
			...
			if (simulationStage == -2) {
				simulationStage++;
				burnoutVelocity = MathUtil.max(status.getRocketVelocity().z,
						0.1);
			...
			if (simulationStage == -1 && status.getRocketVelocity().z >= 0) {
				log.debug("Method 2:  vel=" + status.getRocketVelocity().z
						+ " burnout=" + burnoutVelocity);
				setSimulationProgress(MathUtil.map(
						status.getRocketVelocity().z, burnoutVelocity, 0,
			...
			log.debug("Method 3:  alt=" + status.getRocketPosition().z
		...
		protected void simulationInterrupted(Throwable t) {
			...
			if (t instanceof SimulationLaunchException) {
				...
				DetailDialog.showDetailedMessageDialog(
						SimulationRunDialog.this,
						new Object[] {
								// // Unable to simulate:
								trans.get("SimuRunDlg.msg.Unabletosim"),
								t.getMessage() }, null, simulation.getName(),
				...
				Application.getExceptionHandler().handleErrorCondition(
						"An exception occurred during the simulation", t);
		 ...
		 * @author Sampo Niskanen <sampo.niskanen@iki.fi>
		 */
		private class SimulationProgressListener extends
				AbstractSimulationListener {
			private long time = 0;

			@Override
			public boolean handleFlightEvent(SimulationStatus status,
					FlightEvent event) {
