...
public abstract class SymmetricComponent extends BodyComponent implements BoxBounded, RadialParent {
	...
	/**
	 * Obtain the CG and mass of the symmetric component. We need this method
	 * because subclasses
	 * override getComponentCG() and include mass of shoulders (if any), while
	 ...
	 * see
	 * https://web.mit.edu/8.13/8.13c/references-fall/aip/aip-handbook-section2c.pdf
	 * page 2-41, table 2c-2
	 * Caution! Returns 10/3 times the correct answer. Will need to be corrected at
	 * end
	 * 
	 * @param r1 radius of fore end of frustum
	 * @param r2 radius of aft end of frustum
	...
	private double calculateLongMOI(double l, double r1, double r2, Coordinate cg) {
		...
		// Find the heights of the two cones. Note that the h1 and h2 being calculated
		// here
	...
	protected void calculateProperties() {
		...
		for (int n = 0; n < DIVISIONS; n++) {
			...
			if (filled) {
				...
				// Tiny inaccuracy is introduced on a division where one end is closed and other
				// is open.
	...
	public final SymmetricComponent getPreviousSymmetricComponent() {
		...
		// one last thing -- I could be the child of a ComponentAssembly, and in line
		// with
		...
		if ((grandParent instanceof SymmetricComponent) && inline(grandParent)) {
			...
			// If not, then search for the component before the grandparent
				...
				// NOTE: will be incorrect if the ComponentAssembly is even further to the front
				// than the
				// previous component of the grandparent. But that would be really bad rocket
				// design...
	...
	/**
	 * Checks if parent has component assemblies that have potential previous
	 * components.
	 * A child symmetric component of a component assembly is a potential previous
	 * component if:
	 ...
	 * @param flushOffset an extra offset to be added to check for flushness. This
	 *                    is used when recursively running this
	 *                    method to check children of children of the original
	 *                    parent are flush with the end of the
	 *                    original parent.
	...
	public final SymmetricComponent getNextSymmetricComponent() {
		...
		// One last thing -- I could have a child that is a ComponentAssembly that is in
		// line
		// with me
				...
				for (RocketComponent grandchild : child.getChildren()) {
					if ((grandchild instanceof SymmetricComponent) && inline(grandchild)) {
						...
						// If not, then search for the component after the grandparent
							...
							// than the
							// next component of the grandparent. But that would be really bad rocket
							// design...
	...
	/**
	 * Checks if parent has component assemblies that have potential next
	 * components.
	 * A child symmetric component of a component assembly is a potential next
	 * component if:
	 * - it is inline with the parent
	 ...
	 *                    is used when recursively running this
	 *                    method to check children of children of the original
	 *                    parent are flush with the front of the
	 *                    original parent.
