...
public abstract class FinSet extends ExternalComponent
		implements AxialPositionable, BoxBounded, RingInstanceable, InsideColorComponent {
	...
	public FinSet() {
		...
		this.filletMaterial = Application.getPreferences().getDefaultComponentMaterial(this.getClass(),
				Material.Type.BULK);
	...
	/**
	 * the tab's positioning method variable does not change the internal
	 * representation --
	...
	public void validateFinTabLength() {
		// System.err.println(String.format(" >> Fin Tab Length: %.6f @ %.6f",
		// tabLength, tabOffset));
	...
	/**
	 * Calculates the maximum height that the fin tabs can be, depending on the
	 * parent shape.
	 * 
	...
	/**
	 * Returns the radius of the parent at the front of the fin, or null if no
	 * parent is present.
	 * 
	...
	/**
	 * Returns the radius of the parent at the trailing edge of the fin, or null if
	...
	protected Coordinate calculateFilletVolumeCentroid() {
		...
		for (int index = 1; index < mountPoints.length; index++) {
			...
			final Coordinate segmentCrossSection = calculateFilletCrossSection(this.filletRadius, bodyRadius)
					.setX(xAvg);
	...
	/**
	 * \brief calculate the area-under-the-curve (i.e. the integral) in the form of
	 * a centroid + area
	 * 
	...
	protected static Coordinate calculateCurveIntegral(final Coordinate[] points) {
		...
		for (int index = 1; index < points.length; index++) {
			...
			final double y_avg = (cur.y + prev.y) * 0.5; // TODO: MEDIUM: what if one of the points is below the x-axis?
															// (can produce negative area)
		...
		// Negative weight => make positive. TODO: This is NOT a correct solution, but
		// at least it won't throw an exception...
	...
	/**
	 * The coordinate contains an x,y coordinate of the centroid, relative to the
	 * parent-body-centerline
	...
	/**
	 * copies the supplied areas into a third array, such that the first curve is
	 * copied forward, and the second is copied in reverse.
	 * 
	 * The motivation is to use the two sets of forward points to produce a single
	 * close curve, suitable for an integration operation
	 * 
	 ...
	 * The coordinates are the XY-coordinates of points defining the shape of a
	 * single fin,
	 * where the origin is the leading root edge. Therefore, the first point must be
	 * (0,0,0).
	 ...
	 *
	 * @return points representing the fin-root points, relative to ( x: fin-front,
	 *         y: centerline ) i.e. relto: fin Component reference point
	 */
	 ...
	 */
	private Coordinate[] getMountPoints(final double xStart, final double xEnd, final double xOffset,
		...
		if ((!MathUtil.equals(getCantAngle(), 0))
				|| (parent instanceof Transition) && (((Transition) parent).getShapeType() != Shape.CONICAL)) {
			// the maximum precision to enforce when calculating the areas of fins
			// (especially on curved parent bodies)
			final double xWidth = 0.0025; // width (in meters) of each individual iteration
			divisionCount = (int) Math.ceil(intervalLength / xWidth);

			// When creating body curves, don't create more than this many divisions. --
			// only relevant on very large components
			// a too high division count will cause the 3D render to have invisible faces
			// because it can't deal with the geometry.
		...
		/*
		 * If the front fin point is outside the parent's bounds, and the last point is
		 * still within the parent's bounds,
		 * then we need to add an extra root point at the front of the parent. Same goes
		 * for the last point, but vice versa.
		 * This ensures that fins are drawn correctly on transitions and nose cones (see
		 * GitHub issue #1021 for more info).
		 */
		// Front fin point is outside the parent's bounds and last point is beyond the
		// parent's fore end
		if (xStart < 0 && xEnd > 0) {
			points.add(1, new Coordinate(0, points.get(0).y));
		}
		// End fin point is beyond the parent's aft and first point is still before the
		// parent's aft end
	...
	/**
	 * Returns a y offset that should be applied to the fin root point at the given
	 * x position to account for the fin cant.
	 * 
	 ...
	 * @param xCurr  The x position of the fin root point to calculate the y offset
	 *               for.
	...
	private double getFinCantYOffset(final double xStart, final double xEnd, double xCurr) {
		...
		if (MathUtil.equals(cantAngle, 0) || (xStart > parent.length && xEnd > parent.length)
				|| (xStart < 0 && xEnd < 0)) {
			return 0;
		}
		...
		if (dz >= radius_rot) {
			...
			/*
			 * Simplification of r^2 = (r-dy)^2 + dz^2, given that dz < r. (to derive this,
			 * draw
			 * the cross-section of the body, which is just a circle, with a tangent line
			 * starting on the top of the circle
			 * and reaching a distance dz. Use some Pythagorean theorem and bam, you got
			 * this equation.)
			 */
	...
	/**
	 * Return a list of coordinates defining the geometry of a single fin, including
	 * the parent's body points .
	 */
	 ...
	 * the parent's body points .
	 *
	 * This low res version is for 3D rendering, as a too high resolution would
	 * cause clipping and invisible fin faces.
	 * This should at one point be solved by rendering the fin faces using
	 * triangulation, instead of how it's currently implemented.
	 */
	 ...
	 * the radial distance inwards from the reference point, depending on
	 * positioning method:
	...
	/**
	 * Get the span of a single fin. That is, the length from the root to the tip of
	 * the fin.
	 * 
	...
	public StringBuilder toDebugDetail() {
		...
		if (!this.isTabTrivial()) {
			buf.append(String.format("    TabLength: %6.4f TabHeight: %6.4f @ %6.4f    via: %s\n", tabLength, tabHeight,
					tabPosition, this.tabOffsetMethod));
	...
	private void calculateCM() {
		...
		// set y coordinate: rotate around parent, if single fin; otherwise multiple
		// fins will average out to zero
		if (finCount == 1) {
