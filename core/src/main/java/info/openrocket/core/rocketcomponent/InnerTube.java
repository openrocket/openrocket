...
public class InnerTube extends ThicknessRingComponent
	...
	public void setInstanceCount(final int newCount) {
		log.error("Programmer Error:  cannot set the instance count of an InnerTube directly." +
				...
				new UnsupportedOperationException(
						"InnerTube.setInstanceCount(..) on an" + this.getClass().getSimpleName()));
	}
	...
	/**
	 * Get the cluster scaling as an absolute distance measurement. A value of 0
	 * indicates that the tubes are packed
	 * touching each other, larger values separate the tubes and smaller values pack
	 * inside each other.
	 */
	...
	public List<Coordinate> getClusterPoints() {
		...
		for (int i = 0; i < points.size() / 2; i++) {
			list.add(new Coordinate(0, points.get(2 * i) * separation + yOffset,
					points.get(2 * i + 1) * separation + zOffset));
		}
		return list;
	}
	...
	// throw new BugException("Inconsistent cluster configuration, cluster count=" +
	// count +
	...
	public boolean hasMotor() {
		// the default MotorInstance is the EMPTY_INSTANCE. If we have more than that,
		// then the other instance will have a motor.
	...
	/**
	 * For a given coordinate that represents one tube in a cluster, create an
	 * instance of that tube. Must be called
	 ...
	 *
	 * @return an instance of an inner tube that represents ONE of the clustered
	 *         tubes in the cluster represented
	 *         by <code>theInnerTube</code>
	 */
	public static InnerTube makeIndividualClusterComponent(Coordinate coord, String splitName,
			RocketComponent theInnerTube) {
