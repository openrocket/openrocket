...
public class FlightConfigurableParameterSet<E extends FlightConfigurableParameter<E>> implements Iterable<E> {
	...
	public E get(final int index) {
		if (0 > index) {
			throw new ArrayIndexOutOfBoundsException(
					"Attempt to retrieve a configurable parameter by an index less than zero: " + index);
		}
		if ((0 > index) || (this.map.size() <= index)) {
			throw new ArrayIndexOutOfBoundsException(
					"Attempt to retrieve a configurable parameter with an index larger "
	...
	/*
	 * Clears all configuration-specific settings -- meaning querying the parameter
	 * for any configuration will return the default value.
	 */
	public void reset() {
	...
	public FlightConfigurationId copyFlightConfiguration(FlightConfigurationId oldConfigId,
			FlightConfigurationId newConfigId) {
