...
public abstract class RocketComponent implements ChangeSource, Cloneable, Iterable<RocketComponent> {
	...
	// Because of changes to Java 1.7.0-45's mechanism to construct DataFlavor
	// objects (used in Drag and Drop)
	// We cannot access static members of the Application object in this class.
	// Instead of holding
	// on to the Translator object, we'll just use when we need it.
	// private static final Translator trans = Application.getTranslator();

	/**
	 * A safety mutex that can be used to prevent concurrent access to this
	 * component.
	 */
	...
	/**
	 * Characteristic length of the component. This is used in calculating the
	 * coordinate
	 * transformations and positions of other components in reference to this
	 * component.
	 * This may and should be used as the "true" length of the component, where
	 * applicable.
	...
	/**
	 * How this component is axially positioned, possibly in relative to the parent
	 * component.
	 */
	protected AxialMethod axialMethod = AxialMethod.AFTER;

	/**
	 * Offset of the position of this component relative to the normal position
	 * given by
	 ...
	 * In case (null == parent ): i.e. the Rocket/root component, the position is
	 * constrained to 0,0,0, and is the reference origin for the entire rocket.
	 * Defaults to (0,0,0)
	 */
	...
	private RocketComponent massOverriddenBy = null; // The (super-)parent component that overrides the mass of this
														// component
	...
	private RocketComponent CGOverriddenBy = null; // The (super-)parent component that overrides the CG of this
													// component
	...
	private RocketComponent CDOverriddenBy = null; // The (super-)parent component that overrides the CD of this
													// component
	...
	/**
	 * Used to invalidate the component after calling
	 * {@link #copyFrom(RocketComponent)}.
	 */
	private final Invalidator invalidator = new Invalidator(this);

	/**
	 * List of components that will set their properties to the same as the current
	 * component
	 */
	protected List<RocketComponent> configListeners = new LinkedList<>();

	/**
	 * This is for determining the order in which the component should be drawn in
	 * the 2D views, both
	 * in the side view and in the back view.
	 * Lower values will be placed more in the back, higher values more in the
	 * front.
	...
	/**
	 * Default constructor. Sets the name of the component to the component's static
	 * name
	...
	/**
	 * Static component name. The name may not vary of the parameters, it must be
	 * static.
	 */
	 ...
	 * The unitary moment of inertia is the moment of inertia with the assumption
	 * that
	 ...
	 *
	 * @return <code>true</code> if children can be attached to this component,
	 *         <code>false</code> otherwise.
	 */
	public abstract boolean allowsChildren();

	/**
	 * Test whether the given component type can be added to this component. This
	 * type safety
	 * is enforced by the <code>addChild()</code> methods. The return value of this
	 * method
	 * may change to reflect the current state of this component (e.g. two
	 * components of some
	...
	/**
	 * Test whether the given component can be added to this component. This is
	 * equivalent
	...
	/**
	 * Return a collection of bounding coordinates. The coordinates must be such
	 * that
	 * the component is fully enclosed in their convex hull.
	 * 
	 * Note: this function gets the bounds only for this component. Sub-children
	...
	/**
	 * Shift the coordinates in the array corresponding to radial movement. A
	 * component
	 ...
	 * <p>
	 * The default implementation simply returns the array, and thus produces no
	 * shift.
	 *
	...
	/**
	 * Called when any component in the tree fires a ComponentChangeEvent. This is
	 * by
	...
	/**
	 * Create a string describing the basic component structure from this component
	 * downwards.
	 * 
	 ...
	 * a new component ID, making it a safe copy. This method does not fire any
	 * events.
	 *
	 ...
	 * <p>
	 * This method must be overridden by any component that refers to mutable
	 * objects,
	 * or if some fields should not be copied. This should be performed by
	 * <code>RocketComponent c = super.copyWithOriginalID();</code> and then
	 * cloning/modifying
	...
	public double getComponentCD(double AOA, double theta, double mach, double rollRate) {
		...
		try {
			...
			// This can happen due to a race condition when a loadFrom() action is performed
			// of the rocket (after
			// an undo operation) but the rocket is not yet fully loaded (the sustainer does
			// not yet have the rocket as
			// its parent => getRocket() will not return the rocket, but the sustainer). In
			// that case, just return 0 and
		...
		Map<RocketComponent, AerodynamicForces> aeroData = aerodynamicCalculator.getForceAnalysis(configuration,
				conditions, warnings);
	 ...
	 * <code>false</code>.
	 *
	 * @return whether the current mass, CG, and/or CD override overrides
	 ...
	 *
	 * If we are setting the override false, descend the component tree marking
	 * every
	 * component as not overridden by ancestor.
	 * If in the course of descending the tree we encounter a descendant whose
	 * direct
	 * CD override and overrideSubcomponentsCD flags are both true, descend from
	 * there
	...
	/**
	 * Returns which (super-)parent overrides the mass of this component, or null if
	 * no parent does so.
	 */
	...
	/**
	 * Returns which (super-)parent overrides the CG of this component, or null if
	 * no parent does so.
	 */
	...
	/**
	 * Returns which (super-)parent overrides the CD of this component, or null if
	 * no parent does so.
	 */
	...
	private void updateChildrenMassOverriddenBy() {
		...
		for (RocketComponent c : getAllChildren()) {
			...
			// We need to update overriddenBy in case one of the children components has its
			// subcomponents overridden
	...
	/**
	 * placeholder. This allows code to generally test if this component represents
	 * multiple instances with just one function call.
	 * 
	 ...
	 * 
	 * @return the preset component, or <code>null</code> if this is not based on a
	 *         preset.
	 */
	 ...
	 * 
	 * @return the most compatible ComponentPreset.Type or <code>null</code> if no
	 *         presets are compatible.
	 */
	 ...
	 * preset values.
	 * 
	 * @param preset the preset component to load, or <code>null</code> to clear the
	 *               preset.
	 */
	public final void loadPreset(ComponentPreset preset) {
		 ...
		 * throw new IllegalArgumentException("Attempting to load preset of type " +
		 * preset.getComponentClass()
	 ...
	 * <p>
	 * This method should fire the appropriate events related to the changes. The
	 * rocket
	 * is frozen by the caller, so the events will be automatically combined.
	 * <p>
	 * This method must FIRST perform the preset loading and THEN call
	...
	/**
	 * Clear the current component preset. This does not affect the component
	 * properties
	 * otherwise.
	 */
	 ...
	 * Generally not recommended to directly set the ID, this is done automatically.
	 * Only use this in case you have to.
	 * 
	 * @param newID new ID
	 */
	...
	/**
	 * Get the characteristic length of the component, for example the length of a
	 * body tube
	 * of the length of the root chord of a fin. This is used in positioning the
	 * component
	 * relative to its parent.
	 *
	 * If the length of a component is settable, the class must define the setter
	 * method
	 * itself.
	 */
	...
	public void setAxialMethod(final AxialMethod newAxialMethod) {
		...
		// this variable changes the internal representation, but not the physical
		// position
		// the relativePosition (method) is just the lens through which external code
		// may view this component's position.
		this.axialMethod = newAxialMethod;
	...
	/**
	 * Determine position relative to given position argument. Note: This is a
	 * side-effect free method. No state
	 * is modified.
	 *
	 * @param asMethod the relative positioning method to be used for the
	 *                 computation
	 *
	 * @return double position of the component relative to the parent, with respect
	 ...
	 *
	 * @param newOffset the desired offset of this component, using the components
	 *                  current axial-method
	 */
	...
	protected void setAxialOffset(final AxialMethod requestedMethod, final double requestedOffset) {
		...
		if (null == this.parent) {
			// best-effort approximation. this should be corrected later on in the
	 ...
	 * <p>
	 * For example, the absolute position of any given instance is the parent's
	 * position
	...
	/**
	 * Provides locations of all instances of component relative to this component's
	 * reference point
	 *
	 * <p>
	 * NOTE: the length of this array returned always equals this.getInstanceCount()
	...
	/**
	 * Provides locations of all instances of component *accounting for all parent
	 * instancing*
	 * 
	 * <p>
	 * NOTE: the length of this array MAY OR MAY NOT EQUAL this.getInstanceCount()
	 * --> RocketComponent::getInstanceCount() counts how many times this component
	 * replicates on its own
	 * --> vs. the total instance count due to parent assembly instancing, e.g. a
	 * 2-instance rail button in a
	 * 3-instance pod set will return 6 locations, not 2
	 *
	 * @return Coordinates of all instance locations in the rocket, relative to the
	 *         rocket's origin
	 */
	...
	/**
	 * Provides angles of all instances of component *accounting for all parent
	 * instancing*
	 *
	 * <p>
	 * NOTE: the length of this array MAY OR MAY NOT EQUAL this.getInstanceCount()
	 ...
	 * 3-instance pod set will return 6 locations, not 2
	 *
	 * @return Coordinates of all instance angles in the rocket, relative to the
	 *         rocket's origin
	 *         x-component = rotation around x-axis, y = around y-axis, and z around
	 *         z-axis
	...
	/**
	 * Returns coordinate c in absolute/global/rocket coordinates. Equivalent to
	 * toComponent(c,null).
	 * Input coordinate C is interpreted to be position relative to this component's
	 * *center*, just as
	 ...
	 * @return an array of coordinates describing <code>c</code> in global
	 *         coordinates.
	 */
	...
	// protected static final Coordinate[] rebase(final Coordinate toMove[], final
	// Coordinate source, final Coordinate dest) {
	 ...
	 *
	 * Returns the CG with the weight of the coordinate set to the weight of the
	 * component.
	...
	/**
	 * Return the longitudinal (around the y- or z-axis) moment of inertia of this
	 * component.
	 * The moment of inertia is scaled in reference to the (possibly overridden)
	 * mass
	...
	/**
	 * Return the rotational (around the y- or z-axis) moment of inertia of this
	 * component.
	 * The moment of inertia is scaled in reference to the (possibly overridden)
	 * mass
	 * and is relative to the non-overridden CG.
	 ...
	 * @param trackStage If component is a stage, this check will decide whether the
	 *                   rocket should track that stage (add it to the stageList
	 *                   etc.)
	 ...
	 * <p>
	 * This method may be overridden to enforce more strict component addition
	 * rules.
	...
	public void addChild(RocketComponent component, int index, boolean trackStage) {
		...
		// Ensure that the no loops are created in component tree [A -> X -> Y -> B,
		// B.addChild(A)]
		...
		for (Iterator<RocketComponent> it = component.iterator(false); it.hasNext();) {
			...
			// You only want to change the overriddenBy if the overriddenBy of component
			// changed (i.e. is not null),
			// otherwise you could lose overriddenBy information of the sub-children that
			// have one of this component's
	 ...
	 * @param trackStage If component is a stage, this check will decide whether the
	 *                   rocket should track that stage (remove it to the stageList
	 *                   etc.)
	...
	public final boolean removeChild(RocketComponent component, boolean trackStage) {
		...
		if (children.remove(component)) {
			...
			for (RocketComponent c : component) {
				...
				// component is either
				// this component, or a (super-)parent of this component. Otherwise, you could
				// lose the overrideBy
				// information of sub-children that have one of this component's children as its
				// overrideBy component.
	...
	/**
	 * Fires an AERODYNAMIC_CHANGE, MASS_CHANGE or OTHER_CHANGE event depending on
	 * the
	...
	/**
	 * Returns all the direct children of this component. The result is a clone of
	 * the children list and may be edited.
	 * 
	...
	/**
	 * Returns all the children of this component, including children of
	 * sub-components (children of children).
	 * The order is the same as you would read in the component tree (disregarding
	 * parent-child relations; just top to
	 * bottom).
	 */
	...
	/**
	 * Checks whether this component contains <component> as one of its
	 * (sub-)children.
	 * 
	...
	/**
	 * Get the parent component of this component. Returns <code>null</code> if the
	 * component
	 * has no parent.
	 *
	...
	/**
	 * Iteratively checks whether the list of components contains the parent or
	 * super-parent (parent of parent of parent of...)
	 * of component.
	 * 
	...
	/**
	 * Checks whether all components in the list have the same class as this
	 * component.
	 * 
	 ...
	 * @throws IllegalStateException if we cannot find an AxialStage above
	 *                               <code>this</code>
	 */
	...
	/**
	 * Return all the component assemblies that are a direct/indirect child of this
	 * component
	 * 
	 * @return list of ComponentAssembly components that are a direct/indirect child
	 *         of this component
	 */
	 ...
	 * 
	 * @return list of ComponentAssembly components that are a direct child of this
	 *         component
	 */
	...
	/**
	 * Return all the component assemblies that are a parent or super-parent of this
	 * component
	 * 
	 * @return list of ComponentAssembly components that are a parent or
	 *         super-parent of this component
	 */
	...
	public int getStageNumber() {
		...
		// obviously, this depends on AxialStage overriding <code> .getStageNumber()
		// </code>.
	...
	/**
	 * Find a component with the given ID. The component tree is searched from this
	 * component
	 * down (including this component) for the ID and the corresponding component is
	 * returned,
	...
	/**
	 * Adds a ComponentChangeListener to the rocket tree. The listener is added to
	 * the root
	 * component, which must be of type Rocket (which overrides this method). Events
	 * of all
	...
	/**
	 * Removes a ComponentChangeListener from the rocket tree. The listener is
	 * removed from
	 * the root component, which must be of type Rocket (which overrides this
	 * method).
	 ...
	 * <code>ChangeListener</code>, as <code>ComponentChangeEvent</code> is a
	 * subclass
	 * of <code>ChangeEvent</code>.
	 *
	 * @throws IllegalStateException - if the root component is not a
	 *                               <code>Rocket</code>
	 */
	@Override
	...
	/**
	 * Fires a ComponentChangeEvent on the rocket structure. The call is passed to
	 * the
	...
	/**
	 * Fires a ComponentChangeEvent of the given type. The source of the event is
	 * set to
	 * this component.
	 *
	...
	public boolean addConfigListener(RocketComponent listener) {
		if (isBypassComponentChangeEvent()) {
			// This is a precaution. If you are multi-comp editing and the current component
			// is bypassing events,
	...
	/**
	 * Checks whether this component has been invalidated and should no longer be
	 * used.
	 * This is a safety check that in-place replaced components are no longer used.
	 * All non-trivial methods (with the exception of methods simply getting a
	 * property)
	 * should call this method before changing or computing anything.
	 *
	 * @throws BugException if this component has been invalidated by
	 *                      {@link #copyFrom(RocketComponent)}.
	 */
	 ...
	 * <p>
	 * TODO: Remove this after the "inconsistent internal state" bug has been
	 * corrected
	 */
		...
		for (RocketComponent child : this.children) {
			if (child.parent != this) {
				throw new BugException(
						"Inconsistent component structure detected, child does not have this component " +
	...
	// Check whether the list contains exactly the searched-for component (with ==
	// operator)
	 ...
	 * <p>
	 * The iterator iterates through all children below this object, including
	 * itself if
	 * <code>returnSelf</code> is true. The order of the iteration is not specified
	 * (it may be specified in the future).
	 * <p>
	 * If an iterator iterating over only the direct children of the component is
	 * required,
	 * use <code>component.getChildren().iterator()</code>.
	 *
	 * @param returnSelf boolean value specifying whether the component itself
	 *                   should be
	 *                   returned
	...
	/**
	 * Returns an iterator that iterates over this component, its children and
	 * sub-children.
	 * <p>
	...
	/**
	 * the default implementation is mostly a placeholder here, however in
	 * inheriting classes,
	...
	/**
	 * Helper method to add two points on opposite corners of a box around the
	 * rocket centerline. This box will be (x_max - x_min) long, and 2*r wide/high.
	 */
	...
	/**
	 * Helper method to add four bounds rotated around the given x coordinate at
	 * radius 'r', and 90deg between each.
	 * The X-axis value is <code>x</code> and the radius at the specified position
	 * is
	 * <code>r</code>.
	 */
	...
	/**
	 * Loads the RocketComponent fields from the given component. This method is
	 * meant
	 ...
	 * <p>
	 * The child components in the source tree are copied into the current tree,
	 * however,
	 * the original components should not be used since they represent old copies of
	 * the
	 * components. It is recommended to invalidate them by calling
	 * {@link #invalidate()}.
	 * <p>
	 * This method returns a list of components that should be invalidated after
	 * references
	 * to them have been removed (for example by firing appropriate events). The
	 * list contains
	 * all children and sub-children of the current component and the entire
	 * component
