...
public class FlightConfiguration implements FlightConfigurableParameter<FlightConfiguration>, Monitorable {
	...
	/**
	 * This method flags the specified stage as active, and all other stages as
	 * inactive.
	 * 
	...
	/**
	 * This method flags the specified stage as requested. Substages may be
	 * affected, depending on third parameter
	 *
	 * @param stageNumber       stage number to flag
	 * @param _active           inactive (<code>false</code>) or active
	 *                          (<code>true</code>)
	 * @param activateSubStages whether the sub-stages of the specified stage should
	...
	/**
	 * This method flags the specified stage as requested. Actives the sub-stages of
	 * the specified stage as well.
	 * 
	 * @param stageNumber stage number to flag
	 * @param _active     inactive (<code>false</code>) or active
	 *                    (<code>true</code>)
	 ...
	 * This method is to be used during the import and readout of an OpenRocket
	 * design file.
	 * 
	...
	/**
	 * Returns the InstanceMap of instances that need to be rendered, but are not
	 * present in {@link #getActiveInstances()}.
	 * This is the case for example for a booster that has no children. It is marked
	 * as an inactive stage, but it still needs to be rendered.
	 * 
	 * @return the InstanceMap of instances that need to be rendered, but are not
	 *         present in {@link #getActiveInstances()}.
	 */
	...
	/*
	 * Generates a read-only, instance-aware collection of the components for this
	 * rocket & configuration
	 * 
	 * TODO: swap in this function for the 'getActiveComponents() function, above;
	 * ONLY WHEN READY / MATURE!
	 */
	...
	private InstanceMap getActiveContextListAt(final RocketComponent component, final InstanceMap results,
		...
		for (int currentInstanceNumber = 0; currentInstanceNumber < instanceCount; currentInstanceNumber++) {
			final Transformation offsetTransform = Transformation
					.getTranslationTransform(allOffsets[currentInstanceNumber]);
	 ...
	 * @throws IllegalArgumentException if a motor with the specified ID already
	 *                                  exists.
	 */
	public void addMotor(MotorConfiguration motorConfig) {
		if (motorConfig.isEmpty()) {
			log.error("attempt to add an empty motorConfig! ignoring. ",
					new IllegalArgumentException("empty MotorInstance: " + motorConfig.toDebugDetail()));
		}
	 ...
	 * 
	 * @deprecated Migrate to <FlightConfiguration>.getBoundingBoxAerodynamic(),
	 *             when practical.
	 */
	@Deprecated
	...
	/**
	 * Return the bounding box of the current configuration (of aerodynamic
	 * components).
	 * 
		...
		for (Map.Entry<RocketComponent, java.util.ArrayList<InstanceContext>> entry : map.entrySet()) {
			...
			if (component instanceof BoxBounded) {
				...
				if (instanceBounds.isEmpty()) {
					// probably redundant
					// this component is probably non-physical (like an assembly) or has invalid
					// bounds. Skip.
					continue;
				}
	...
	/**
	 * Returns the length of the rocket configuration (only aerodynamic components),
	 * from the foremost bound X-coordinate
	...
	/**
	 * Returns the length of the rocket configuration (all components), from the
	 * foremost bound X-coordinate
	 * to the aft-most X-coordinate. The value is cached.
	 *
	 * @return the length of the rocket in the X-direction.
	 */
	...
	public FlightConfiguration clone(Rocket rocket) {
		...
		clone.preloadStageActiveness = this.preloadStageActiveness == null ? null
				: new HashMap<>(this.preloadStageActiveness);
	...
	public FlightConfiguration copy(final FlightConfigurationId newId) {
		...
		copy.preloadStageActiveness = this.preloadStageActiveness == null ? null
				: new HashMap<>(this.preloadStageActiveness);
