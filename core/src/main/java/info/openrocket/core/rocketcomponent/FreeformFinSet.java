...
public class FreeformFinSet extends FinSet {
	...
	// this class uses certain features of 'ArrayList' which are not implemented in
	...
	// attempts to set a fin value any larger than this will be snapped to this max
	// value
	...
	/**
	 * Converts a point of this fin set to edit into a point for a config listener
	 * to edit.
	 *
	 * The editing is as follows:
	 * 1) Editing the first point of this fin set will always edit the first point
	 * of the listener set
	 * 2) Editing the last point of this fin set will always edit the last point of
	 * the listener set
	 * 3) Editing any other point of this fin set will edit the corresponding point
	 * of the listener set, except
	 * for when the current point is not the last of this set, but it is the last of
	 * the listener set. In that
	 ...
	 * @return the point index of the listener fin set that needs to be edited.
	 *         Returns -1 if the listener's point should not be edited.
	 */
	private int getConfigListenerPointIdx(FreeformFinSet listener, int index) {
		...
		if (index == this.points.size() - 1) {
			...
			if (index == listener.getPointCount() - 1) {
				// If editing the last point of the listener, but not the last point of this fin
				// set, don't edit the listener
				return -1;
			} else {
				// Index-wise editing
				return index;
	 ...
	 * @throws IllegalFinPointException if removing would result in invalid fin
	 *                                  planform
	 */
	...
	/**
	 * The first point is assumed to be at the origin. If it isn't, it will be moved
	 * there.
	 * 
	 ...
	 * all of the other points the corresponding distance back, relative to the
	 * first.
	...
	public void setPoint(final int index, final double xRequest, final double yRequest)
		...
		// if the new x,y would cause a fin larger than our max-size, limit the new
		// request:
	...
	private void clampLastPoint(final double xDelta) {
		...
		if (IGNORE_SMALLER_THAN < Math.abs(yDelta)) {
			// i.e. if it delta is close enough above OR is inside the body. In either case,
			// snap it to the body.
	...
	/**
	 * Check if the line segment from targetIndex to targetIndex+1 intersects with
	 * any other part of the fin.
	 * 
	...
	private boolean intersects(final int targetIndex) {
		if ((points.size() - 2) < targetIndex) {
			...
			// throw new IndexOutOfBoundsException("request validate of non-existent fin
			// edge segment: " + targetIndex + "/" + points.size());
		}
		...
		for (int comparisonIndex = targetIndex + 1; comparisonIndex < (points.size() - 1); ++comparisonIndex) {
			if (2 > Math.abs(targetIndex - comparisonIndex)) {
				...
				// nor can adjacent line segments intersect with each other, because they share
				// a common endpoint.
				continue;
			}
			final Point2D.Double pc1 = new Point2D.Double(points.get(comparisonIndex).x, points.get(comparisonIndex).y); // p1
			final Point2D.Double pc2 = new Point2D.Double(points.get(comparisonIndex + 1).x,
					points.get(comparisonIndex + 1).y); // p2

			// special case for when the first and last points are co-located.
			if ((0 == targetIndex) && (points.size() == comparisonIndex + 2)
					&& (IGNORE_SMALLER_THAN > Math.abs(pt1.distance(pc2)))) {
				continue;
			}

			final Line2D.Double comparisonLine = new Line2D.Double(pc1, pc2);
			if (targetLine.intersectsLine(comparisonLine)) {
				log.warn(String.format("Found intersection at %d-%d and %d-%d", targetIndex, targetIndex + 1,
