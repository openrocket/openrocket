...
/**
 * Base for all rocket components. This is the "starting point" for all rocket
 * trees.
 * It provides the actual implementations of several methods defined in
 * RocketComponent
 * (eg. the rocket listener lists) and the methods defined in RocketComponent
 * call these.
 * It also defines some other methods that concern the whole rocket, and helper
 * methods
...
public class Rocket extends ComponentAssembly {
	...
	public Rocket() {
		...
		final FlightConfiguration defaultConfig = new FlightConfiguration(this,
				FlightConfigurationId.DEFAULT_VALUE_FCID);
		configSet = new FlightConfigurableParameterSet<>(defaultConfig);
		this.selectedConfiguration = defaultConfig;
	}

	/**
	 * Return a bounding box enveloping the rocket. By definition, the bounding box
	 * is a convex hull.
	 *
	 ...
	 * {@link #getAerodynamicModID()} {@link #getTreeModID()}, which change every
	 * time
	 * a mass change, aerodynamic change, or tree change occur. Even though the
	 * values
	 * of the different modification ID's may be equal, they should be treated
	 * totally
	 * separate.
	 * <p>
	...
	/**
	 * Get the topmost stage (including boosters), only taking into account active
	 * stages from the flight configuration.
	 * 
	...
	/**
	 * Get the bottommost stage, only taking into account active stages from the
	...
	/**
	 * Make a shallow copy of the Rocket structure. This method is exposed as public
	 * to allow
	 * for undo/redo system functionality.
	 * 
	 * note: the <hashmap>.clone() function returns a shallow copy-- which is
	...
	public Rocket copyWithOriginalID() {
		...
		// the default value needs to be explicitly set, because it has different
		// semantics
	...
	/**
	 * Load the rocket structure from the source. The method loads the fields of
	 * this
	 * Rocket object and copies the references to siblings from the
	 * <code>source</code>.
	 * The object <code>source</code> should not be used after this call, as it is
	 * in
	 * an illegal state!
	 * <p>
	 * This method is meant to be used in conjunction with undo/redo functionality,
	 * and therefore fires an UNDO_EVENT, masked with all applicable
	 * mass/aerodynamic/tree
	 * changes.
	 */
	...
	/**
	 * Creates a new EventListenerList for this component. This is necessary when
	 * cloning
	 * the structure.
	 */
	 ...
	 * @param ids  IDs of the flight configurations to update, or null to update
	 *             all.
	...
	private void notifyAllListeners(final ComponentChangeEvent cce) {
		...
		for (EventListener l : list) {
			{ // vvvv DEVEL vvvv
				// System.err.println("notifying listener. (type=
	...
	public void freeze() {
		...
		if (freezeList == null) {
			...
			Application.getExceptionHandler()
					.handleErrorCondition("Attempting to freeze Rocket when it is already frozen, " +
							"freezeList=" + freezeList);
		}
	}

	/**
	 * Thaws a frozen rocket structure and fires a combination of the events fired
	 * during
	 * the freeze. The event type is a combination of those fired and the source is
	 * the
	...
	/**
	 * Return the currently selected configuration. This should be used in the user
	 * interface
	...
	/**
	 * Return a flight configuration. If the supplied id does not have a specific
	 * instance, the default is returned.
	 *
	...
	/**
	 * Return a flight configuration. If the supplied index is out of bounds, an
	 * exception is thrown.
	...
	public void setFlightConfiguration(final FlightConfigurationId fcid, FlightConfiguration newConfig) {
		checkState();
		if (fcid.hasError()) {
			log.error("attempt to set a 'fcid = config' with a error fcid.  Ignored.",
					new IllegalArgumentException("error id:" + fcid));
			return;
		}
	...
	/**
	 * Allows only <code>AxialStage</code> components to be added to the type
	 * Rocket.
	 */
	@Override
	...
	/**
	 * STUB. would enable the monitoring, relay and production of events in this
	 * rocket instance.
	 */
