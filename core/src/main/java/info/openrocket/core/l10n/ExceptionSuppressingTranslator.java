...
/**
 * A translator that suppresses MissingResourceExceptions and handles them
 * gracefully.
 * For the first missing key this class calls the exception handler, and
 * afterwards
	...
	private static synchronized void handleError(String key, MissingResourceException e) {
		if (!errorReported) {
			...
			Application.getExceptionHandler().handleErrorCondition(
					"Can not find translation for '" + key + "' locale=" + Locale.getDefault(), e);
		}
	}
