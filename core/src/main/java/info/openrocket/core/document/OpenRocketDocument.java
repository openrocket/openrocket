...
public class OpenRocketDocument implements ComponentChangeListener, StateChangeListener {
	...
	private final List<String> file_extensions = Arrays.asList("ork", "ork.gz", "rkt", "rkt.gz"); // Possible extensions
																									// of an OpenRocket
																									// document
	/**
	...
	// The Photo Settings will be saved in the core module as a map of key values
	...
	/**
	 * The undo history of the rocket. Whenever a new undo position is created while
	 * the
	...
	/**
	 * The position in the undoHistory we are currently at. If modifications have
	 * been
	 * made to the rocket, the rocket is in "dirty" state and this points to the
	 * previous
	 * "clean" state.
	 */
	...
	public void addCustomExpression(CustomExpression expression) {
		if (customExpressions.contains(expression)) {
			log.info(Markers.USER_MARKER, "Could not add custom expression " + expression.getName()
					+ " to document as document already has a matching expression.");
		}
	...
	// TODO:LOW:this leaves the object custom expression exposed, is it supposed to
	// be like that?
	/**
	 * 
	 * @return
	...
	/**
	 * @returns a set of all the flight data types defined or available in any way
	 *          in the rocket document
	 */
	...
	/**
	 * returns the File handler object for the document without the file extension
	 * (e.g. '.ork' removed)
	 * 
	 ...
	 * 
	 * @param configId the Flight Configuration Id that dictates which simulations
	 *                 should be removed
	 */
	...
	/**
	 * Adds an undo point at this position. This method should be called *before*
	 * any
	 * action that is to be undoable. All actions after the call will be undone by a
	 * single "Undo" action.
	 * <p>
	 * The description should be a short, descriptive string of the actions that
	 * will
	 * follow. This is shown to the user e.g. in the Edit-menu, for example
	 * "Undo (Modify body tube)". If the actions are not known (in general should
	 * not
	 * be the case!) description may be null.
	 * <p>
	 * If this method is called successively without any change events occurring
	 * between the
	...
	/**
	 * Start a time-limited undoable operation. After the operation
	 * {@link #stopUndo()}
	 ...
	 * {@link #startUndo(String)} has been called before any other undo operations
	 * are
	 * performed.
	 */
	 ...
	 * 
	 * @return the description what would be undone, or <code>null</code> if
	 ...
	 * 
	 * @return the description of what would be redone, or <code>null</code> if
	 *         description unavailable.
	 */
	...
	/**
	 * Log a non-fatal undo/redo error or inconsistency. Reports it to the user the
	 * first
	 * time it occurs, but not on subsequent times. Logs automatically the undo
	 * system state.
	 */
	...
	/**
	 * Return a copy of this document. The rocket is copied with original ID's, the
	 * default
	 * motor configuration ID is maintained and the simulations are copied to the
	 * new rocket.
	 * No undo/redo information or file storage information is maintained.
	 * 
	 * This function is used from the Optimization routine to store alternatives of
	 * the same rocket.
	 * For now we can assume that the copy returned does not have any of the
	 * attachment factories in place.
	 * 
