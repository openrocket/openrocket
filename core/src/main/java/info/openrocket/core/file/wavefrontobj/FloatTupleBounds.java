...
/**
 * A class for storing the minimum and maximum float tuple values to keep track
 * of the bounds of a model.
 */
public class FloatTupleBounds {
    ...
    /**
     * Default constructor. Initializes the bounds to the maximum and minimum values
     * of a float.
     */
    ...
    public void updateBounds(FloatTuple tuple) {
        min = new DefaultFloatTuple(Math.min(min.getX(), tuple.getX()), Math.min(min.getY(), tuple.getY()),
                Math.min(min.getZ(), tuple.getZ()));
        max = new DefaultFloatTuple(Math.max(max.getX(), tuple.getX()), Math.max(max.getY(), tuple.getY()),
                Math.max(max.getZ(), tuple.getZ()));
    }
