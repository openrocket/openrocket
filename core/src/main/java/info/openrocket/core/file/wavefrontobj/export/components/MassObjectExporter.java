...
public class MassObjectExporter extends RocketComponentExporter<MassObject> {
    ...
    private void generateMesh(int numSides, int numStacks, InstanceContext context) {
        ...
        for (int j = 0; j <= numStacks; j++) {
            ...
            if (j == 0 || j == numStacks) {
                ...
                for (int i = 0; i < numSides; i++) {
                    ...
                    // Normal
                    if (Double.compare(r, component.getRadius()) == 0) { // If in cylindrical section, use cylinder
                                                                         // normals
        ...
        for (int i = 0; i < numSides; i++) {
            ...
            int[] normalIndices = vertexIndices.clone(); // For a smooth surface, the vertex and normal indices are the
                                                         // same
            ...
            ObjUtils.offsetIndex(vertexIndices, startIdx); // Only do this after normals are added, since the vertex
                                                           // indices are used for normals
        ...
        for (int j = 0; j < numStacks - 2; j++) {
            for (int i = 0; i < numSides; i++) {
                ...
                int[] normalIndices = vertexIndices.clone(); // For a smooth surface, the vertex and normal indices are
                                                             // the same
                int[] texCoordIndices = new int[] {
                        j + j * numSides + i,
                        j + 1 + (j + 1) * numSides + i,
