...
public class RASPMotorLoader extends AbstractMotorLoader {
	...
	public List<ThrustCurveMotor.Builder> load(InputStream stream, String filename, boolean removeDelayFromDesignation)
			throws IOException {
		return load(new InputStreamReader(stream, getDefaultCharset()), filename, removeDelayFromDesignation);
	}

	/**
	 * Load a <code>Motor</code> from a RASP file specified by the
	 * <code>Reader</code>.
	 * The <code>Reader</code> is responsible for using the correct charset.
	 * <p>
	 * The CG is assumed to be located at the center of the motor casing and the
	 * mass
	 * is calculated from the thrust curve by assuming a constant exhaust velocity.
	 * 
	 * @param reader                     the source of the file.
	 * @param removeDelayFromDesignation if true, the motor delay will be left out
	 *                                   of the motor designation (e.g. "B6" instead
	 *                                   of "B6-0")
	...
	public List<ThrustCurveMotor.Builder> load(Reader reader, String filename, boolean removeDelayFromDesignation)
		...
		try (BufferedReader in = new BufferedReader(reader)) {
			...
			main: while (line != null) { // Until EOF
				...
				pieces = split(line);
				if (pieces.length != 7) {
					throw new IOException("Illegal file format. Motor header line must contain 7 fields:\n" +
			...
			throw new IOException("Illegal file format. Could not convert value to a number.\n" +
