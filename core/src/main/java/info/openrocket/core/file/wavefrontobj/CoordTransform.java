...
/**
 * Interface for classes that can convert location and rotation coordinates from
 * the OpenRocket coordinate system
 * to a custom OBJ coordinate system.
 * OpenRocket uses a left-handed coordinate system with the forward (y-)axis
 * pointing up, the depth (z-)axis pointing away from the viewer,
 * and the longitudinal (x-)axis pointing to the right (in the side view). Its
 * origin is also at the tip of the rocket.
 *
...
public class CoordTransform {
     ...
     * 
     * @param xAxis     the OpenRocket axis that corresponds to the transformed
     *                  x-axis
     * @param yAxis     the OpenRocket axis that corresponds to the transformed
     *                  y-axis
     * @param zAxis     the OpenRocket axis that corresponds to the transformed
     *                  z-axis
     *                  Example: you want the transformed coordinate system to have
     *                  the z-axis as the longitudinal axis (= OpenRocket x-axis)
     *                  and want its direction to be from the bottom of the rocket
     *                  to the tip of the rocket (opposite direction
     *                  of the OpenRocket x-axis), then you must pass Axis.X_MIN as
     *                  the xAxis parameter.
     *                  You must also add an offset to the origin of the transformed
     *                  coordinate system, so that it starts
     *                  at the bottom of the rocket => set origZOffs to the length
     *                  of the rocket.
     * @param origXOffs the x-offset of the origin of the OBJ coordinate system,
     *                  <b>in the OpenRocket coordinate system</b>
     * @param origYOffs the y-offset of the origin of the OBJ coordinate system,
     *                  <b>in the OpenRocket coordinate system</b>
     * @param origZOffs the z-offset of the origin of the OBJ coordinate system,
     *                  <b>in the OpenRocket coordinate system</b>
     */
     ...
     * Create a new coordinate system converter.
     * 
     * @param axialAxis   the OBJ axis that corresponds to the OpenRocket axial
     *                    (x-)axis
     * @param forwardAxis the OBJ axis that corresponds to the OpenRocket forward
     *                    (y-)axis
     * @param origXOffs   the x-offset of the origin of the OBJ coordinate system,
     *                    <b>in the OpenRocket coordinate system</b>
     * @param origYOffs   the y-offset of the origin of the OBJ coordinate system,
     ...
     * The returned coordinates will also be shifted towards the origin of the OBJ
     * coordinate system.
     * E.g. if x = 0, but the origin of the OBJ coordinate system is at x = 10, then
     * the returned x-coordinate will be 10.
     * (that is in the case that the OpenRocket x-axis is the same as the OBJ
     * x-axis, that no conversion is needed, but that's just for the easy example)
     * 
    ...
    /**
     * Converts the location coordinates from OpenRocket to OBJ coordinates, without
     * the offset of the origin.
     * This is useful for when you want the relative position of a vertex in the
     * OpenRocket coordinate system to be the
     * same as the relative position of the converted vertex to the OBJ coordinate
     * system.
     * </p>
     * E.g. if you have a vertex (0, 1, 2) in OpenRocket, then the converted vertex
     * could be (2, 1, 0) in OBJ coordinates,
     * even you would have an x-axis origin offset of e.g. 10.
     * </p>
     * This is different from {@link #convertLoc(double, double, double)}, where the
     * returned coordinates would
     * be (2, 1, 10), due to the origin x offset.
     * 
     * @param x OpenRocket x-coordinate
     * @param y OpenRocket y-coordinate
     * @param z OpenRocket z-coordinate
     * @return the location coordinates in OBJ coordinates, with the origin set to
     *         the OpenRocket origin (the tip of the rocket)
     */
    ...
    /**
     * Returns the offset of the origin of the OBJ coordinate system for the given
     * axis.
     * 
     * @param axis the axis to get the offset for
     * @return the offset of the origin of the OBJ coordinate system for the given
     *         axis
     */
    ...
    /**
     * Returns the equivalent axis for the x-axis in the OpenRocket coordinate
     * system (axial axis).
     * ! The direction is relative to the OpenRocket x-axis ! For instance, if the
     * OBJ coordinate system uses the z-axis
     * that runs in the opposite direction of the OR x-axis (z-axis runs from the
     * bottom of the rocket to the tip), you
     * must return Axis.Z_MIN.
     * 
     * @return the equivalent axis for the x-axis in the OpenRocket coordinate
     *         system (axial axis)
     */
