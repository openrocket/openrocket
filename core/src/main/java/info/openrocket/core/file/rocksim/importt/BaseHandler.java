...
/**
 * An abstract base class that handles common parsing. All Rocksim component
 * handlers are subclassed from here.
 *
 * @param <C> the specific RocketComponent subtype for which the concrete
 *            handler can create
 */
public abstract class BaseHandler<C extends RocketComponent> extends AbstractElementHandler {
	...
	public void endHandler(String element, HashMap<String, String> attributes, String content, WarningSet warnings)
			throws SAXException {
		/*
		 * Because of the order of XML elements in Rocksim, not all information is known
		 * at the time it really needs
		 * to be acted upon. So we keep temporary instance variables to be used here at
	 ...
	 * Some G10 fiberglass materials are in cubic units, other G10 fiberglass is in
	 * square units. And due to a Rocksim
	...
	/**
	 * If the Rocksim component does not override the mass, then create a Material
	 * based upon the density defined for
	 * that component. This *should* result in a consistent representation of Cg
	 * between Rocksim and OpenRocket.
	 *
	 * @param component       the component
	 * @param type            the type of the material
	 * @param density         the density in g/cm^3
	 * @param definedMaterial the material that is currently defined on the
	 *                        component; used only to get the name as it
	 *                        appears in Rocksim
	 */
	...
	/**
	 * Some CG positions in Rocksim do not correspond to the CG position reference
	 * in OpenRocket.
	 *
	 * @param theCG the CG value to really use when overriding CG on the OpenRocket
	 *              component
	 */
	...
	/**
	 * Set the material onto an instance of RocketComponent. This is done because
	 * only some subtypes of RocketComponent
	 ...
	 * @param material  the material to be set on the component (defined by
	 *                  getComponent())
	 */
