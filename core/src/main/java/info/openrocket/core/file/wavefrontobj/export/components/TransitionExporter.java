...
public class TransitionExporter extends RocketComponentExporter<Transition> {
     ...
     * 
     * @param numSides         the number of sides to use (= number of vertices in
     *                         the circumferential direction)
     * @param numStacks        the number of stacks to use (= number of vertices in
     *                         the longitudinal direction)
     * @param offsetRadius     offset radius from the transition radius
     * @param isOutside        true if the mesh is on the outside of the rocket,
     *                         false if it is on the inside
    ...
    private void addTransitionMesh(int numSides, int numStacks, double offsetRadius, boolean isOutside,
        ...
        // Other meshes may have been added to the obj, so we need to keep track of the
        // starting indices
        ...
        final double actualLength = estimateActualLength(offsetRadius, dxBase); // Actual length of the transition (due
                                                                                // to reduced step size near the
                                                                                // fore/aft end)

        // Get the location where the fore/aft shoulder would end (due to its thickness)
        final double shoulderMargin = 0.001; // Margin to prevent the shoulder from being too close to the transition
                                             // end
        ...
        int actualNumStacks = 0; // Number of stacks added, deviates from set point due to reduced step size near
                                 // the tip (does not include aft/fore tip rings)
        while (x <= (float) component.getLength()) {
            ...
            /*
             * This is where things get a bit complex. Since transitions can have a fore and
             * aft tip, we need to handle
             * this separately from the rest of the mesh. Normal stack rings will eventually
             * be drawn using quads, but
             * transition tips will use an aft/fore tip vertex and then use triangles to
             * connect the tip to the stack rings.
             * Additionally, we need to store the fore ring and aft ring vertices to
             * eventually close the outside and inside
             * mesh. For this, we need to set isAftRing and isForeRing.
             * 
             * The following rules apply:
             * 1. If actualNumStacks == 0, r == 0 and rNext == 0, skip this ring (you're at
             * the fore end, before the fore tip)
             * 2. If actualNumStacks == 0, r == 0 and rNext > 0, add a single vertex at the
             * center (= fore tip)
             * 3. If r > 0 and rNext == 0, set isForeRing = true & add a single vertex at
             * the center at the next x position (= aft tip)
             * 4. If actualNumStacks > 0, r == 0 and rNext == 0, break the loop (you're at
             * the aft tip)
            ...
            if (Double.compare(r, RADIUS_EPSILON) > 0) { // Don't just compare to 0, otherwise you could still have
                                                         // really small rings that could be better replaced by a tip
                // For the inside transition shape if we're in the shoulder base region, we need
                // to skip adding rings,
                // because this is where the shoulder base will be
                if (!isOutside) {
                    if (hasForeShoulder && x < xForeShoulder) {
                        // If the current ring is before the fore shoulder ring and the next ring is
                        // after, clamp the
                        ...
                        // The aft shoulder point is between this and the next ring, so clamp the x
                        // value to the shoulder height
                        // and add the ring
                ...
                // If the current radius is larger than 0, and the next radius is
                // (approximately) a tip,
                ...
                if (Double.compare(rNext, RADIUS_EPSILON) <= 0 && Double.compare(x, component.getLength()) != 0) {
                    ...
                    addQuadVertices(numSides, foreRingVertices, aftRingVertices, r, rNext, x, xNext, isForeRing,
                            isAftRing, isOutside);
                    actualNumStacks++;
                }
            }
    ...
    private void addTipFaces(int numSides, boolean isOutside, boolean isForeTip, int startIdx, int normalsStartIdx,
            int texCoordsStartIdx) {
        ...
        for (int i = 0; i < numSides; i++) {
            ...
            if (isForeTip) {
                ...
                ObjUtils.offsetIndex(vertexIndices, startIdx); // Do this last, otherwise the normal indices will be
                                                               // wrong
            }
            // Aft tip
            else {
                ...
                // No need to offset the indices, because we reference the last vertex (this
                // caused a lot of debugging frustration hmfmwl)
            }
    ...
    private void addQuadFaces(int numSides, int numStacks, int startIdx, int normalsStartIdx, int texCoordsStartIdx,
            boolean isOutside) {
    ...
    private void addShoulders(int nrOfSides, List<Integer> outsideForeRingVertices,
            List<Integer> outsideAftRingVertices,
    ...
    private void addShoulder(float shoulderRadius, float shoulderLength, float shoulderThickness, boolean isCapped,
        ...
        if (!(isCapped || isFilled)) {
            startIdx = obj.getNumVertices();
            CylinderExporter.addCylinderMesh(obj, transformer, null, innerCylinderRadius,
                    shoulderLength + shoulderThickness,
        ...
        if (isForeSide) {
            DiskExporter.closeDiskMesh(obj, transformer, null, outerCylinderForeVertices, innerCylinderForeVertices,
                    false, true);
        } else {
            DiskExporter.closeDiskMesh(obj, transformer, null, outerCylinderAftVertices, innerCylinderAftVertices,
                    false, false);
        }

        // Generate transition outer disk (no. 5)
        if (isForeSide) {
            DiskExporter.closeDiskMesh(obj, transformer, null, outerRingVertices, outerCylinderAftVertices, false,
                    true);
        } else {
            DiskExporter.closeDiskMesh(obj, transformer, null, outerRingVertices, outerCylinderForeVertices, false,
                    false);
        }
        ...
        if (!isFilled) { // If the transition is filled, there is no inside
            if (isForeSide) {
                DiskExporter.closeDiskMesh(obj, transformer, null, innerRingVertices, innerCylinderAftVertices, false,
                        false);
            } else {
                DiskExporter.closeDiskMesh(obj, transformer, null, innerRingVertices, innerCylinderForeVertices, false,
                        true);
            }
        }
    }
    ...
    private void closeFace(List<Integer> outerVertices, List<Integer> innerVertices, boolean isTopFace) {
        ...
        DiskExporter.closeDiskMesh(obj, transformer, null, outerVertices, filledCap ? null : innerVertices, false,
                isTopFace);
    }

    /**
     * Due to the offsetRadius, the length of the transition to be drawn can be
     * smaller than the actual length of the transition,
     * because the offsetRadius causes the mesh to "shrink". This method estimates
     * the length of the transition to be drawn.
     * 
