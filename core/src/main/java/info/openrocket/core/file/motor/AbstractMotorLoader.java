...
public abstract class AbstractMotorLoader implements MotorLoader {
	 ...
	 * {@inheritDoc}
	 * <p>
	 * This method delegates the reading to the loaded from the Reader using the
	 * charset
	...
	protected static List<Double> calculateMass(List<Double> time, List<Double> thrust,
		...
		for (double dm : deltam) {
			total -= dm * scale;
			// to correct negative mass error condition: (caused by rounding errors in the
			// above loop)
			if (total < 0) {
				total = 0;
			}
	...
	/**
	 * Helper method to remove a delay (or plugged) from the end of a motor
	 * designation,
	 * if present.
	 * 
	...
	protected static void finalizeThrustCurve(List<Double> time, List<Double> thrust,
		...
		for (int i = 0; i < time.size() - 1; i++) {
			while ((i < time.size() - 1) &&
				...
				System.out.println("\twarning:  deleting duplicate data point time[" + i + 1 + "]=" + time.get(i + 1)
						+ ", thrust=" + thrust.get(i + 1));
				time.remove(i);
				thrust.remove(i);
				for (List l : lists) {
					l.remove(i);
