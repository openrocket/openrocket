...
class MassObjectHandler extends PositionDependentHandler<MassObject> {
	...
	/**
	 * The Rocksim Mass length fudge factor. Rocksim completely exaggerates the
	 * length of a mass object to the point
	 * that it looks ridiculous in OpenRocket. This fudge factor is here merely to
	 * get the typical mass object to
	 * render in the OpenRocket UI with it's bounds mostly inside it's parent. The
	 * odd thing about it is that Rocksim
	 * does not expose the length of a mass object in the UI and actually treats
	 * mass objects as point objects - not 3
	 ...
	 */
	public MassObjectHandler(DocumentLoadingContext context, RocketComponent c, WarningSet warnings)
	...
	public void closeElement(String element, HashMap<String, String> attributes, String content, WarningSet warnings)
		...
		try {
			...
			if (RockSimCommonConstants.KNOWN_CG.equals(element)) {
				// Setting the CG of the Mass Object to 0 is important because of the different
				// ways that Rocksim and
				// OpenRocket treat mass objects. Rocksim treats them as points (even though the
				// data file contains a
				// length) and because Rocksim sets the CG of the mass object to really be
				// relative to the front of
				// the parent. But that value is already assumed in the position and position
				// value for the component.
				// Thus it needs to be set to 0 to say that the mass object's CG is at the point
				// of the mass object.
				super.setCG(0);
			}
	...
	/**
	 * Rocksim does not have a separate entity for Shock Cords. It has to be
	 * inferred. Sometimes the typeCode
	 * indicates it's a shock cord, but most times it does not. This is due to bugs
	 * in the Rocksim Component and
	 * Material databases. Try to infer a shock cord based on it's length and it's
	 * material type. It's somewhat
	 * arbitrary, but if the mass object's length is more than twice the length of
	 * it's parent component and it's a LINE
	...
	/**
	 * If it appears that the mass object is a shock cord, then create an OR shock
	 * cord instance.
	 *
	...
	private void mapMassObjectAsShockCord(final String element, final HashMap<String, String> attributes,
		...
		// Rocksim does not distinguish between total length of the cord and the packed
		// length. Fudge the
	...
	/**
	 * Get the component this handler is working upon. This changes depending upon
	 * the type of mass object.
	 *
	 * @return a component
	 */
	@Override
	...
	/**
	 * Get the required type of material for this component. Does not apply to
	 * MassComponents, but does apply to Shock
	 * Cords.
	 *
	 * @return LINE
	 */
