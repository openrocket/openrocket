...
public class PolygonExporter {
    ...
    /**
     * Add a polygon mesh to the obj. It is drawn in the OpenRocket XY plane with
     * the bottom left corner at the OpenRocket origin.
     * 
     ...
     * @param pointLocationsX The x locations of the points --> NOTE: points should
     *                        follow a clockwise direction
     * @param pointLocationsY The y locations of the points --> NOTE: points should
     *                        follow a clockwise direction
    ...
    public static void addPolygonMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
        ...
        // Other meshes may have been added to the group, so we need to keep track of
        // the starting indices
        int startIdx = obj.getNumVertices();
        int normalsStartIdx = obj.getNumNormals();

        // NOTE: "front" is the side view with the normal pointing towards the viewer,
        // "back" is the side with the normal pointing away from the viewer
        ...
        for (int i = 0; i < pointLocationsX.length; i++) {
            ...
            ObjUtils.offsetIndex(normalIndices, normalsStartIdx + 2); // Offset by 2 to skip the bottom and top faces
                                                                      // normals
