...
class FlightDataBranchHandler extends AbstractElementHandler {
	...
	public FlightDataBranchHandler(String name, String typeList, SingleSimulationHandler simHandler,
		...
		for (int i = 0; i < split.length; i++) {
			...
			types[i] = matching;
			// types[i] = FlightDataType.getShapeType(typeName, matching.getSymbol(),
			// matching.getUnitGroup());
		}
	...
	// Note: this way of doing it requires that custom expressions always come
	// before flight data in the file,
	...
	private FlightDataType findFlightDataType(String name) {
		...
		// Replace deprecated 'Position upwind' with new 'Position North of launch'
		// option
		...
		log.warn("Could not find the flight data type '" + name
				+ "' used in the XML file. Substituted type with unknown symbol and units.");
