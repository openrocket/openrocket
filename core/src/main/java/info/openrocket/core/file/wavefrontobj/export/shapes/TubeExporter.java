...
public class TubeExporter {
     ...
     * It is drawn in the origin of the OBJ coordinate system. The longitudinal axis
     * is positive, converted OpenRocket x-axis.
     * 
     ...
     *                          Set to 0, together with aftInnerRadius, to create a
     *                          solid tube
     * @param aftInnerRadius    The inner radius of the aft (bottom) of the tube
     *                          Set to 0, together with foreInnerRadius, to create a
     *                          solid tube
     * @param length            The length of the tube
     * @param numSides          The number of sides of the tube
     * @param foreOuterVertices A list to add the indices of the fore (top) outer
     *                          vertices to, or null if the indices are not needed
     * @param aftOuterVertices  A list to add the indices of the aft (bottom) outer
     *                          vertices to, or null if the indices are not needed
     * @param foreInnerVertices A list to add the indices of the fore (top) inner
     *                          vertices to, or null if the indices are not needed
     * @param aftInnerVertices  A list to add the indices of the aft (bottom) inner
     *                          vertices to, or null if the indices are not needed
     */
    public static void addTubeMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
        ...
        // We need to store the vertices to create the closing disks, so we'll create
        // lists if they do not exist
        ...
        boolean hasForeThickness = Float.compare(foreOuterRadius, foreInnerRadius) > 0
                && Float.compare(foreInnerRadius, 0) > 0;
        boolean hasAftThickness = Float.compare(aftOuterRadius, aftInnerRadius) > 0
                && Float.compare(aftInnerRadius, 0) > 0;
    ...
    public static void addTubeMesh(DefaultObj obj, CoordTransform transformer, String groupName,
        ...
        addTubeMesh(obj, transformer, groupName, outerRadius, outerRadius, innerRadius, innerRadius, length,
                LOD.getNrOfSides(outerRadius),
                null, null, null, null);
    }
}
