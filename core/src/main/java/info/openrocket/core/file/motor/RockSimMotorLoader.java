...
import info.openrocket.core.file.simplesax.NullElementHandler;
...
import info.openrocket.core.motor.ThrustCurveMotor.Builder;
...
public class RockSimMotorLoader extends AbstractMotorLoader {
	...
	/**
	 * Load a <code>Motor</code> from a RockSim motor definition file specified by
	 * the
	 * <code>Reader</code>. The <code>Reader</code> is responsible for using the
	 * correct
	 * charset.
	 * <p>
	 * If automatic CG/mass calculation is used, then the CG is assumed to be
	 * located at
	 * the center of the motor casing and the mass is calculated from the thrust
	 * curve
	...
	private static class RSEMotorHandler extends AbstractElementHandler {
		...
		public Builder getMotor() throws SAXException {
			...
			CoordinateIF[] cgArray = new CoordinateIF[n];
			for (int i = 0; i < n; i++) {
