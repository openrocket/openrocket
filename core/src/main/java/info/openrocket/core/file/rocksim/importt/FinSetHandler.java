 ...
 * first (in Rocksim file format, the fin
 * shape type is in the middle of the XML structure), and because we're using
 * SAX not DOM, all of the fin
 * characteristics are kept here until the closing FinSet tag. At that point,
 * <code>asOpenRocket</code> method is called
...
class FinSetHandler extends AbstractElementHandler {
	...
	/**
	 * The OpenRocket Position which gives the absolute/relative positioning for
	 * location.
	 */
	...
	/**
	 * The angle the fins have been rotated from the y-axis, if looking down the
	 * tube, in radians.
	 */
	...
	public void closeElement(String element, HashMap<String, String> attributes, String content, WarningSet warnings)
		...
		try {
			...
			if (RockSimCommonConstants.LOCATION_MODE.equals(element)) {
				...
				// If the location is loaded before the axialMethod, we still need to correct
				// for the different relative distance directions
				if (locationLoaded) {
					if (axialMethod == AxialMethod.BOTTOM) {
						location = -location;
					}
	...
	public void endHandler(String element, HashMap<String, String> attributes,
		...
		if (component.isCompatible(finSet)) {
			...
			if (!override && finSet.getCrossSection().equals(FinSet.CrossSection.AIRFOIL)) {
				// Override mass anyway. This is done only for AIRFOIL because Rocksim does not
				// compute different
				// mass/cg for different cross sections, but OpenRocket does. This can lead to
				// drastic differences
				// in mass. To counteract that, the cross section value is retained but the
				// mass/cg is overridden
				// with the calculated values from Rocksim. This will best approximate the
				// Rocksim design in OpenRocket.
	...
	public FinSet asOpenRocket(WarningSet warnings) {
		...
		// All TTW tabs in Rocksim are relative to the front of the fin, so set an
		// offset if the parent's fore radius is larger than the aft radius.
	...
	/**
	 * Convert a Rocksim string that represents fin plan points into an array of
	 * OpenRocket coordinates.
	 *
	 * @param newPointList a comma and pipe delimited string of X,Y coordinates from
	 *                     Rocksim. This is of the format:
	 * 
	 *                     <pre>
	 * x0,y0|x1,y1|x2,y2|...
	 *                     </pre>
