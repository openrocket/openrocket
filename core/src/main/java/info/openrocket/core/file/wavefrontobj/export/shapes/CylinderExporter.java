...
public class CylinderExporter {
     ...
     * @param transformer      The coordinate system transformer to use to switch
     *                         from the OpenRocket coordinate system to a custom OBJ
     *                         coordinate system
     * @param groupName        The name of the group to add the mesh to, or null if
     *                         no group should be added (use the active group)
     ...
     * @param solid            Whether the cylinder should be solid (true) or hollow
     *                         (false)
     *                         NOTE: Culling is not really thought of for the hollow
     *                         cylinder; this mode is really meant to be
     *                         combined with other objects
     * @param isOutside        Whether the cylinder is an outside face (true) or
     *                         inside face (false)
    ...
    public static void addCylinderMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
        ...
        for (int i = 0; i < numSides; i++) {
            ...
            ObjUtils.offsetIndex(vertexIndices, startIdx + offset); // ! Only add offset here, otherwise you mess up the
                                                                    // indices for the normals
    ...
    public static void addCylinderMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
            ...
            List<Integer> foreRingVertices, List<Integer> aftRingVertices) {
        addCylinderMesh(obj, transformer, groupName, radius, length, numSides, solid, true, foreRingVertices,
                aftRingVertices);
    }
    ...
    public static void addCylinderMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
            ...
            List<Integer> foreRingVertices, List<Integer> aftRingVertices) {
        addCylinderMesh(obj, transformer, groupName, radius, length, nrOfSlices, solid, isOutside, foreRingVertices,
                aftRingVertices);
    }
    ...
    public static void addCylinderMesh(@NotNull DefaultObj obj, @NotNull CoordTransform transformer, String groupName,
            ...
            List<Integer> foreRingVertices, List<Integer> aftRingVertices) {
        addCylinderMesh(obj, transformer, groupName, radius, length, nrOfSlices, solid, foreRingVertices,
                aftRingVertices);
    }
    ...
    public static void generateRingVertices(DefaultObj obj, CoordTransform transformer,
        ...
        for (int i = 0; i < numSides; i++) {
            ...
            obj.addNormal(transformer.convertLocWithoutOriginOffs(nx, ny, nz)); // This kind of normal ensures the
                                                                                // object is smoothly rendered (like the
                                                                                // 'Shade Smooth' option in Blender)
            ...
            float v = isOutside ? (xMax - x) / xMax : x / xMax; // For some reason, the texture is vertically flipped in
                                                                // OR for inside cylinders. Don't really like it, but it
                                                                // is what it is
