...
class DoubleSetter implements Setter {
	 ...
	 * @param parameters    (optional) extra parameter set to use for the setter
	 *                      method.
	 */
	...
	/**
	 * Set the double value, or if the value equals the special string, use the
	 * special setter and set it to true. If the input string contains more
	 * information
	 * besides the special string, you can specify which separator should be used
	 * for
	 * this extra information. The part before the separator is then the special
	 * string
	...
	/**
	 * Set a double value of the default configuration of a FlightConfiguration
	 * object.
	 * 
	...
	public void set(RocketComponent c, String s, HashMap<String, String> attributes,
		...
		// Extract special string and data if s contains multiple data elements,
		...
		if (!special.equalsIgnoreCase(specialString) || (args != null && args.length > 1)) {
			try {
				...
				if (configGetter != null) {
					FlightConfigurableParameterSet<?> config = (FlightConfigurableParameterSet<?>) configGetter
							.invoke(c);
