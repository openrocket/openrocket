...
/**
 * A "simple SAX" XML reader. This system imposes the limit that an XML element
 * may
 * contain either textual (non-whitespace) content OR additional elements, but
 * not
 * both. This holds true for both the OpenRocket and RockSim design formats and
 * the
 * RockSim engine definition format.
 * <p>
 * The actual handling is performed by subclasses of {@link ElementHandler}. The
 * initial handler is provided to the
 * {@link #readXML(InputSource, ElementHandler, WarningSet)}
 * method.
 *
...
public class SimpleSAX {
	 ...
	 * @param warnings       a warning set to store warning (cannot be
	 *                       <code>null</code>).
