 ...
 * <p>
 * Note that unlike normal listener implementations, this list does NOT allow
 * the
 * exact same listener (equality using ==) twice. While adding a listener twice
 * to
 * a event source would in principle be valid, in practice it's most likely a
 * bug.
...
public class ListenerList<T> implements Invalidatable, Iterable<T> {
	 ...
	 * already is in the list (checked by the equality operator ==). This method
	 * throws
	...
	/**
	 * Remove the specified listener from the list. The listener is removed based on
	 * the
	 ...
	 * a copy of the iterator list, so {@link #addListener(Object)} and
	 * {@link #removeListener(Object)}
	 * may be called while iterating the list without effect on the iteration. The
	 * returned
	...
	/**
	 * Invalidate this listener list. Invalidation removes all listeners from the
	 * list.
	 * After invalidation {@link #addListener(Object)} will throw an exception, the
	 * other
