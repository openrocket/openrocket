...
/**
 * Defines an affine transformation of the form A*x+c, where x and c are
 * Coordinates and
 * A is a 3x3 matrix.
 * 
 * The Transformations are immutable. All modification methods return a new
 * transformation.
 * 
...
public class Transformation implements java.io.Serializable {
	...
	public static final Transformation PROJECT_XY = new Transformation(
			new double[][] { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 0 } });
	public static final Transformation PROJECT_YZ = new Transformation(
			new double[][] { { 0, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } });
	public static final Transformation PROJECT_XZ = new Transformation(
	...
	/**
	 * Transforms all coordinates in a Collection. The original coordinate elements
	 * are
	 * removed from the set and replaced with the transformed ones. The Collection
	 * given
	 ...
	 * transformation result.transform(c) will equal
	 * this.transform(other.transform(c)).
	 * 
	...
	public Transformation applyTransformation(Transformation other) {
		...
		for (int i = 0; i < 3; i++) {
			...
			y = rotation[i][Y];
			z = rotation[i][Z];
			combined.rotation[i][X] = x * other.rotation[X][X] + y * other.rotation[Y][X] + z * other.rotation[Z][X];
			combined.rotation[i][Y] = x * other.rotation[X][Y] + y * other.rotation[Y][Y] + z * other.rotation[Z][Y];
			combined.rotation[i][Z] = x * other.rotation[X][Z] + y * other.rotation[Y][Z] + z * other.rotation[Z][Z];
		}
		return combined;
	}
