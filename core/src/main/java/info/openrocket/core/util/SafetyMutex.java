...
public abstract class SafetyMutex {
	...
	/**
	 * Return a new instance of a safety mutex. This returns an actual
	 * implementation
	 * or a bogus implementation depending on whether safety checks are enabled or
	 * disabled.
	 * 
	...
	/**
	 * Verify that this mutex is unlocked, but don't lock it. This has the same
	 * effect
	 * as <code>mutex.lock(); mutex.unlock();</code> and is useful for methods that
	 * return
	 ...
	 * a ConcurrencyException is thrown. The location parameter is used to
	 * distinguish
	 * the locking location, and it should be e.g. the method name.
	 * 
	 * @param location a string describing the location where this mutex was locked
	 *                 (cannot be null).
	 * 
	 * @throws ConcurrencyException if this mutex is already locked.
	 */
	...
	/**
	 * Unlock this mutex. If this mutex is not locked at the position of the
	 * parameter
	 * or was locked by another thread than the current thread an error is raised,
	 * but an exception is not thrown.
	 * <p>
	 * This method is guaranteed never to throw an exception, so it can safely be
	 * used in finally blocks.
	 * 
	 * @param location a location string matching that which locked the mutex
	 * @return whether the unlocking was successful (this normally doesn't need to
	 *         be checked)
	 */
	public abstract boolean unlock(String location);

	/**
	 * Bogus implementation of a safety mutex (used when safety checking is not
	 * performed).
	 */
	...
	static class ConcreteSafetyMutex extends SafetyMutex {
		private static final boolean STORE_LOCKING_LOCATION = (System
				.getProperty("openrocket.debug.mutexlocation") != null);
		...
		// longingLocation is set when lockingThread is, if STORE_LOCKING_LOCATION is
		// true
		...
		public synchronized boolean unlock(String location) {
			try {
				...
				Application.getExceptionHandler()
						.handleErrorCondition("An exception occurred while unlocking a mutex, " +
		...
		/**
		 * Check that the internal state of the mutex (lockingThread vs. locations) is
		 * correct.
		 */
		private void checkState(boolean throwException) {
			...
			if ((lockingThread == null) ^ (locations.isEmpty())) {
				// Clear the mutex only after error() has executed (and possibly thrown an
				// exception)
				try {
		...
		/**
		 * Raise an error. The first occurrence is passed directly to the exception
		 * handler,
