...
/**
 * A class that performs certain memory-management operations for debugging
 * purposes.
 * For example, complex objects that are being discarded and that should be
 * garbage-collectable
 * (such as dialog windows) should be registered for monitoring by calling
 * {@link #collectable(Object)}. This will allow monitoring whether the object
 * really is
 * garbage-collected or whether it is retained in memory due to a memory leak.
 * Only complex objects should be registered due to the overhead of the
 * monitoring.
 * 
...
public final class MemoryManagement {
	...
	/**
	 * Purge cleared references every this many calls to
	 * {@link #collectable(Object)}
	 */
	private static final int PURGE_CALL_COUNT = 1000;

	/**
	 * Storage of the objects. This is basically a mapping from the objects (using
	 * weak references)
	 * to
	 */
	...
	/**
	 * Mark an object that should be garbage-collectable by the GC. This class will
	 * monitor
	 * whether the object actually gets garbage-collected or not by holding a weak
	 * reference
	 * to the object.
	 * 
	...
	/**
	 * Return a list of MemoryData objects corresponding to the objects that have
	 * been
	 * registered by {@link #collectable(Object)} and have not been
	 ...
	 * 
	 * @return a list of MemoryData objects for objects that have not yet been
	 *         garbage-collected.
	 */
	...
	/**
	 * Register a new ListenerList object. This can be used to monitor freeing of
	 * listeners
	 * and find memory leaks. The objects are held by a weak reference, allowing
	 * them to be
	 * garbage-collected.
	 * 
	...
	/**
	 * Return a list of listener list objects corresponding to the objects that have
	 * been
	 * registered by {@link #registerListenerList(ListenerList)} and have not been
	 * garbage-collected yet.
	 * This method first calls <code>System.gc()</code> multiple times to attempt to
	 * force any remaining garbage collection.
	 * 
	 * @return a list of listener list objects that have not yet been
	 *         garbage-collected.
	 */
