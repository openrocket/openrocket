 ...
 * <p>
 * The implementation must fire change events when the minimum and maximum
 * ranges
...
public interface SimulationModifier extends ChangeSource {
	...
	/**
	 * Return the unit group used for the values returned by
	 * {@link #getCurrentSIValue(Simulation)} etc.
	 * 
	...
	/**
	 * Return the current scaled value. This is normally within the range [0...1],
	 * but
	 * can be outside the range if the current value is outside of the min and max
	 * values.
	 * 
	 ...
	 * This method can be used to ensure the simulation or rocket are properly
	 * configured.
	 * 
	 * @param simulation
	 */
	...
	/**
	 * Compare whether this SimulationModifier is equivalent to another simulation
	 * modifier.
	 * "Equivalent" means that the simulation modifier corresponds to the same
	 * modification in
	 * another rocket instance (e.g. the same modification on another rocket
	 * component that
