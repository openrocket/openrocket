...
import info.openrocket.core.util.Statistics;

/**
 * A customized implementation of the parallel multidirectional search algorithm
 * by Dennis and Torczon.
 * <p>
 * This is a parallel pattern search optimization algorithm. The function
 * evaluations are performed
 * using an ExecutorService. By default a ThreadPoolExecutor is used that has as
 * many thread defined
...
public class MultidirectionalSearchOptimizer implements FunctionOptimizer, Statistics {
			...
			while (continueOptimization) {
				...
				if (accept(reflection, currentValue)) {
					...
					if (useExpansion) {
						...
						/*
						 * Assume expansion to be unsuccessful, queue next reflection while computing
						 * expansion.
						 */
						createReflection(simplex, reflection);
					...
					/*
					 * Assume coordinate search to be unsuccessful, queue contraction step while
					 * computing.
					 */
					halveStep(simplex);
					functionExecutor.compute(simplex);
	...
	private void halveStep(List<Point> base) {
		...
		for (int i = 1; i < base.size(); i++) {
			...
			/*
			 * new = (old - current)*0.5 + current = old*0.5 + current*0.5 = (old +
			 * current)*0.5
			 */
