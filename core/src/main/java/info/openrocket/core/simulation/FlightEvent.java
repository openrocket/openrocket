...
public class FlightEvent implements Comparable<FlightEvent> {
	...
	public enum Type {
		...
		/**
		 * Ignition of a motor. Source is the motor mount the motor of which has
		 * ignited,
		...
		/**
		 * Burnout of a motor. Source is the motor mount the motor of which has burnt
		 * out,
		 * and the data is the MotorId of the motor instance.
		 */
		 ...
		 * which has exploded its ejection charge, and data is the MotorId of the motor
		 * instance.
		 */
		EJECTION_CHARGE(trans.get("FlightEvent.Type.EJECTION_CHARGE")),
		/**
		 * Separation of a stage. Source is the stage which is being separated from the
		 * upper stages.
		 */
		...
		/**
		 * Opening of a recovery device. Source is the RecoveryComponent which has
		 * opened.
		 */
	 ...
	 * sorting is performed on stages; lower (numerically higher) stage first.
	 * Tertiary
	...
	@Override
	public String toString() {
		return "FlightEvent[type=" + type.name() + ",time=" + time + ",source=" + source + ",data="
				+ String.valueOf(data) + "]";
