...
public class BasicEventSimulationEngine implements SimulationEngine {
	...
	public FlightData simulate(SimulationConditions simulationConditions) throws SimulationException {
		...
		currentStatus.getEventQueue()
				.add(new FlightEvent(FlightEvent.Type.LAUNCH, 0, simulationConditions.getRocket()));
		{
			// main simulation branch
			final String branchName = simulationConfig.getRocket().getTopmostStage(currentStatus.getConfiguration())
					.getName();
	...
	private FlightDataBranch simulateLoop() throws SimulationException {
		...
		try {
			...
			while (handleEvents()) {
				...
				if (!currentStatus.isLiftoff()) {
					...
					if ((currentStatus.getRocketPosition().z < MathUtil.EPSILON) && !currentStatus.isLanded()) {
						...
						// addEvent(new FlightEvent(FlightEvent.Type.SIMULATION_END,
						// currentStatus.getSimulationTime()));
					}
			...
			currentStatus.getFlightData()
					.addEvent(new FlightEvent(FlightEvent.Type.EXCEPTION, currentStatus.getSimulationTime(),
							currentStatus.getConfiguration().getRocket(), e.getLocalizedMessage()));
	...
	private boolean handleEvents() throws SimulationException {
		...
		for (event = nextEvent(); event != null; event = nextEvent()) {
			...
			// If I get an event other than ALTITUDE and SIMULATION_END after I'm on the
			...
			for (MotorClusterState state : currentStatus.getActiveMotors()) {
				if (state.testForIgnition(currentStatus.getConfiguration(), event)) {
					...
					// log.info(" Because of "+event.getShapeType().name()+" @"+event.getTime()+"
					// from: "+event.getSource().getName());
			...
			// TODO: LOW: check if deprecated function getActiveComponents needs to be
			// replaced
			for (RocketComponent c : currentStatus.getConfiguration().getActiveComponents()) {
				if (!(c instanceof RecoveryDevice))
					continue;
				DeploymentConfiguration deployConfig = ((RecoveryDevice) c).getDeploymentConfigurations()
						.get(this.fcid);
			...
			switch (event.getType()) {
				...
				case BURNOUT: {
					...
					// log.debug( " adding EJECTION_CHARGE event for motor
					// "+motorState.getMotor().getDesignation()+" on stage
					// "+stage.getStageNumber()+": "+stage.getName());
					log.debug(" detected Motor Burnout for motor " + motorState.getMotor().getDesignation() + "@ "
							+ event.getTime() + "  on stage " + stage.getStageNumber() + ": " + stage.getName());

					double delay = motorState.getEjectionDelay();
					if (motorState.hasEjectionCharge()) {
						addEvent(new FlightEvent(FlightEvent.Type.EJECTION_CHARGE,
				...
				case STAGE_SEPARATION: {
					...
					if (currentStatus.getConfiguration().isStageActive(stageNumber - 1)) {
						...
						boosterStatus.setFlightData(new FlightDataBranch(boosterStage.getName(), boosterStage,
								currentStatus.getFlightData()));
				...
				case RECOVERY_DEVICE_DEPLOYMENT:
					...
					if (currentStatus.getConfiguration().isStageActive(n)) {
						...
						// If we haven't already reached apogee, then we need to compute the actual
						// coast time
				...
				case TUMBLE:
					...
					final boolean tooMuchThrust = currentStatus.getFlightData()
							.getLast(FlightDataType.TYPE_THRUST_FORCE) > THRUST_TUMBLE_CONDITION;
					if (tooMuchThrust) {
		...
		if (!currentStatus.isMotorIgnited()) {
			// TODO MEDIUM: display this as a warning to the user (e.g. highlight the cell
			// in the simulation panel in red and a hover: 'make sure the motor ignition is
