...
public abstract class AbstractEulerStepper extends AbstractSimulationStepper {
	...
	public void step(SimulationStatus status, double maxTimeStep) throws SimulationException {
		...
		final double jerk = Math.abs(linearAcceleration.sub(status.getRocketAcceleration())
				.multiply(1.0 / status.getPreviousTimeStep()).length());
		...
		log.trace("time " + data.getLast(FlightDataType.TYPE_TIME) + ", altitude "
				+ data.getLast(FlightDataType.TYPE_ALTITUDE) + ", velocity "
				+ data.getLast(FlightDataType.TYPE_VELOCITY_Z));
	}
