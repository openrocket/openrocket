...
public class RK4SimulationStepper extends AbstractSimulationStepper {
	...
	public void step(SimulationStatus simulationStatus, double maxTimeStep) throws SimulationException {
		...
		/*
		 * Start with previous time step which is used to compute the initial thrust
		 * estimate.
		...
		/*
		 * Compute the initial thrust estimate. This is used for the first time step
		 * computation.
		 */
		 ...
		 * dt[0]: the user-specified time step (or 1/5th of it if still on the launch
		 * rod)
		 ...
		 * 
		 * The limits #5 and #6 are required since near the steady-state roll rate the
		 * roll rate
		 * may oscillate significantly even between the sub-steps of the RK4
		 * integration.
		 * 
		...
		/*
		 * Compute the correct thrust for this time step. If the original thrust
		 * estimate differs more
		 * than 10% from the true value then recompute the RK4 step 1. The 10% error in
		 * step 1 is
		 * diminished by it affecting only 1/6th of the total, so it's an acceptable
		 * error.
		 */
		...
		status2.setRocketOrientationQuaternion(status.getRocketOrientationQuaternion()
				.multiplyLeft(Quaternion.rotation(k1.rv.multiply(store.timestep / 2))));
		...
		status2.setRocketOrientationQuaternion(status2.getRocketOrientationQuaternion()
				.multiplyLeft(Quaternion.rotation(k2.rv.multiply(store.timestep / 2))));
		...
		status2.setRocketOrientationQuaternion(status2.getRocketOrientationQuaternion()
				.multiplyLeft(Quaternion.rotation(k3.rv.multiply(store.timestep))));
		...
		// Store data
		// TODO: MEDIUM: Store acceleration etc of entire RK4 step, store should be
		// cloned or something...
	...
	private RK4Parameters computeParameters(RK4SimulationStatus status, DataStore dataStore)
		...
		// Call post-listeners
		store.accelerationData = SimulationListenerHelper.firePostAccelerationCalculation(status,
	...
	/**
	 * Calculate the linear and angular acceleration at the given status. The
	 * results
	 * are stored in the fields {@link #linearAcceleration} and
	 * {@link #angularAcceleration}.
	 * 
	 * @param status the status of the rocket.
	 * @throws SimulationException
	 */
	private AccelerationData calculateAcceleration(RK4SimulationStatus status, DataStore store)
			throws SimulationException {
		...
		// If still on the launch rod, project acceleration onto launch rod direction
		// and
		...
		return new AccelerationData(null, null, store.linearAcceleration, store.angularAcceleration,
				status.getRocketOrientationQuaternion());
	}

	/**
	 * Calculate the aerodynamic forces into the data store. This method also
	 * handles
	...
	private void calculateForces(RK4SimulationStatus status, DataStore store) throws SimulationException {
		...
		// Add very small randomization to yaw & pitch moments to prevent over-perfect
		// flight
	...
	private void calculateFlightConditions(RK4SimulationStatus status, DataStore store)
		...
		if (store.flightConditions != null) {
			...
			store.lateralPitchRate = Math.hypot(store.flightConditions.getPitchRate(),
					store.flightConditions.getYawRate());
			return;
		}
	...
	private void storeData(RK4SimulationStatus status, DataStore store) {
		...
		if (null != store.motorMass) {
			...
			// data.setValue(FlightDataType.TYPE_MOTOR_LONGITUDINAL_INERTIA,
			// store.motorMassData.getLongitudinalInertia());
			// data.setValue(FlightDataType.TYPE_MOTOR_ROTATIONAL_INERTIA,
			// store.motorMassData.getRotationalInertia());
		}
		if (store.rocketMass != null) {
		...
		if (status.isLaunchRodCleared() && store.forces != null) {
			if (store.rocketMass != null && store.flightConditions != null) {
				data.setValue(FlightDataType.TYPE_PITCH_MOMENT_COEFF,
						store.forces.getCm() - store.forces.getCN() * store.rocketMass.getCM().x
								/ store.flightConditions.getRefLength());
				data.setValue(FlightDataType.TYPE_YAW_MOMENT_COEFF,
						store.forces.getCyaw() - store.forces.getCside() * store.rocketMass.getCM().x
								/ store.flightConditions.getRefLength());
			}
