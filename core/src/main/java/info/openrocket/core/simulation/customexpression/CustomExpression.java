...
public class CustomExpression implements Cloneable {
	...
	/*
	 * Sets the symbol string. This is the short, locale independent symbol for this
	 * whole expression
	 */
	...
	private String subTimeRanges(String str) {
		...
		// for each match, make a new custom expression (in subExpressions) with a
		// hashed name
		// and replace the expression and variable in the original expression string
		// with [hash].
		while (m.find()) {
	...
	private String subTimeIndexes(String str) {
		...
		// find any matches of the time-indexed variable notation, e.g. m[1.2] for mass
		// at 1.2 sec
		Pattern p = Pattern.compile(variableRegex() + "\\[[^:]*?\\]");
		Matcher m = p.matcher(str);
	...
	/*
	 * Builds the expression, done automatically during evaluation. Logs any errors.
	...
	/*
	 * Builds a specified expression, log any errors and returns null in case of
	 * error.
	 */
	protected Calculable buildExpression(ExpressionBuilder b) {
		...
		try {
			...
			log.info(Markers.USER_MARKER, "Unparsable expression. Could not build custom expression " + this.toString()
					+ ". " + e1.getMessage());
			return null;
		}
	...
	/*
	 * Evaluate the expression using the last variable values from the simulation
	 * status.
	...
	public Variable evaluate(SimulationStatus status) {
		...
		try {
			...
			log.info(Markers.USER_MARKER,
					"Unable to calculate expression " + this.expression + " due to empty stack exception");
		}
	 ...
	 * If the unit matches a SI unit string then the datatype will have the
	 * corresponding unitgroup.
	...
	public FlightDataType getType() {
		...
		// log.debug(this.getClass().getSimpleName()+" returned type "+type.getName()+"
		// (" + type.getSymbol() + ")" );
	...
	public void addToDocument() {
		...
		if (!expressions.isEmpty()) {
			...
			if (expressions.contains(this)) {
				log.info(Markers.USER_MARKER, "Expression already in document. This unit : " + this.getUnit()
						+ ", existing unit : " + expressions.get(0).getUnit());
				return;
			}
		}
	...
	/*
	 * Removes this expression from the document, replacing it with a given new
	 * expression
	 */
	...
	@Override
	/*
	 * Clone method makes a deep copy of everything except the reference to the
	 * document.
	 * If you want to apply this to another simulation, set simulation manually
	 * after cloning.
	 * 
