...
/**
 * General substitutor for motor configurations. This currently includes
 * substitutions for
 ...
 * - a combination of motors and manufacturers, e.g. {motors | manufacturers} ->
 * "M1350-0 | AeroTech"
 * You can choose which comes first and what the separator is. E.g.
 * {manufacturers, motors} -> "AeroTech, M1350-0".
 *
 * <p>
 * This substitutor is added through injection. All substitutors with the
 * "@Plugin" tag in the formatting package will
...
public class MotorConfigurationSubstitutor implements RocketSubstitutor {
    ...
    // Map containing substitution words and their corresponding replacement
    // strings.
    ...
    public boolean containsSubstitution(String input) {
        Pattern pattern = Pattern.compile("\\" + SUB_START + "(.*?[^\\s])\\" + SUB_END); // ensures non-whitespace
                                                                                         // content
    ...
    private List<String> combineSubstitutesForStages(Rocket rocket, FlightConfiguration config,
        ...
        for (AxialStage stage : rocket.getStageList()) {
            ...
            // Parse through all the substitutes (motors, manufacturers, etc.) for each
            // stage to build a combined stage substitution
            int idx = 0;
            for (Map<AxialStage, List<String>> substituteMap : stageSubstitutes) {
                ...
                // If this is not the first substitute, add a separator between the different
                // substitutes (motor, manufacturer, etc.)
        ...
        public Map<AxialStage, List<String>> substitute(Rocket rocket, FlightConfigurationId fcid) {
            ...
            for (AxialStage stage : rocket.getStageList()) {
                ...
                for (RocketComponent child : stage.getAllChildren()) {
                    ...
                    // Plus other conditions :) But I'm not gonna bore you with those details. The
                    // goal
                    // of code documentation is to not waste a programmers time by making them read
                    // too
                    // much text when you can word something in a more concise way. I think I've
                    // done that
