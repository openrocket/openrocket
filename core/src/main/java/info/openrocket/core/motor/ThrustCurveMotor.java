...
public class ThrustCurveMotor implements Motor, Comparable<ThrustCurveMotor>, Serializable {
	...
	public static class Builder {
		...
		/**
		 * Simplify a motor designation, if possible. This attempts to reduce the
		 * designation
		 * into a simple letter + number notation for the impulse class and average
		 * thrust.
		 * 
		 * @param str the designation to simplify
		 * @return the simplified designation, or the string itself if the format was
		 *         not detected
		 */
		...
		public ThrustCurveMotor build() {
			...
			// throw new IllegalArgumentException("Curve starts at thrust " +
			// motor.thrust[0]);
			// }
			//
	 ...
	 * fractional part: [0,1]: weighting to interpolate between the given index and
	 * the next index.
	 * 
	 ...
	 * {@inheritDoc}
	 * <p>
	 * NOTE: In most cases you want to examine the motor type of the
	 * ThrustCurveMotorSet,
	...
	/**
	 * for testing. In practice, the return value should generally match the
	 * parameter value, (except for error conditions)
	 * 
	...
	private void computeStatistics() {
		...
		if (thrust[0] >= thrustLimit)
			...
			if (MathUtil.equals(thrust[startPos - 1], thrust[startPos])) {
				...
				burnStart = MathUtil.map(thrustLimit, thrust[startPos - 1], thrust[startPos], time[startPos - 1],
						time[startPos]);
			}
		}
	...
	/**
	 * Return a String representation of a delay time. If the delay is
	 * {@link #PLUGGED_DELAY},
	 * returns "P".
	 * 
