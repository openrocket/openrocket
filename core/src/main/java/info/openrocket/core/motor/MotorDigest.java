 ...
 * uniquely identifies a motor (like a hash code or checksum). Two motors that
 * have
 * the same digest behave similarly with a very high probability. The digest can
 * therefore be used to identify motors that otherwise have the same
 * specifications.
 * <p>
 * The digest only uses a limited amount of precision, so that rounding errors
 * won't
...
public class MotorDigest {
	...
	public enum DataType {
		...
		TIME_ARRAY(0, 1000),
		/**
		 * Mass data for a few specific points (normally initial and empty mass) (in
		 * 0.1g)
		 */
