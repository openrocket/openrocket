...
public class MassCalculation {
	...
	/**
	 * NOTE: Multiple enums may map to the same settings. This allows a caller to
	 * use the
	...
	public MassCalculation copy(final RocketComponent _root, final Transformation _transform) {
		return new MassCalculation(this.type, this.config, this.simulationTime, this.activeMotorList, _root, _transform,
				this.analysisMap);
	}
	...
	public String toCMDebug() {
		return String.format("cm= %.6fg@[%.6f,%.6f,%.6f]", centerOfMass.weight, centerOfMass.x, centerOfMass.y,
				centerOfMass.z);
	}

	// public String toMOIDebug(){
	// return String.format("I_cm=[ %.8f, %.8f, %.8f ]", inertia.getIxx(),
	// inertia.getIyy(), inertia.getIzz() ));
	// }
	...
	private MassCalculation calculateMountData() {
		...
		// System.err.println(String.format("%-40s|Motor: %s.... Mass @%f = %.6f",
		// prefix, motorConfig.toDescription(), motorTime, eachMass ));

		// coordinates in rocket frame; Ir, It about CoM.
		final Coordinate clusterLocalCM = new Coordinate(mountXPosition + motorXPosition + eachCMx, 0, 0,
		...
		// if more than 1 motor => motors are not at the centerline => adjust via
	...
	MassCalculation calculateStructure() {
		...
		// System.err.println(String.format( "%s>>[%s]....", prefix,
		// component.getName()));
		// }
		...
		if (this.config.isComponentActive(component)) {
			...
			if (null != analysisMap) {
				...
				if (component instanceof ComponentAssembly) {
					...
					// For actual components, record the mass of the component, and disregard
					// children
			...
			// // vvv DEBUG
			// if( 0 < compCM.weight ) {
			// System.err.println(String.format( "%s....componentData: %s", prefix,
			// compCM.toPreciseString() ));
			// }
		}
		...
		// System.err.println(String.format( "%s....<< return data @ %s: %s", prefix,
		// component.getName(), this.toCMDebug() ));
		// }
		// // ^^^ DEBUG
	...
	MassCalculation calculateMotors() {
		...
		if (component.isMotorMount()) {
			...
			// // vvv DEBUG
			// if( 0 < motor.getMass() ) {
			// System.err.println(String.format( "%s........++ motorData: %s", prefix,
			// propellant.toCMDebug()));
			// }
		...
		if (MIN_MASS < children.getMass()) {
			this.merge(children);
			// System.err.println(String.format( "%s....assembly mass (incl/children): %s",
			// prefix, this.toCMDebug()));
		}

		// // vvv DEBUG
		// if( this.config.isComponentActive(component) && 0 < this.getMass() ) {
		// System.err.println(String.format( "%s....<< return assemblyData: %s (tree
		// @%s)", prefix, this.toCMDebug(), component.getName() ));
		// }
		// // ^^^ DEBUG
	 ...
	 * (1) calculate overall Center-of-Mass (CM) first (down inline with
	 * data-gathering)
