...
public class UpdateInfoRetriever {
	...
	public static class UpdateInfoFetcher extends Thread {
		...
		public void runUpdateFetcher() throws UpdateCheckerException {
			...
			JsonObject latestObj = getLatestReleaseJSON(jsonArr, preTag, filterTags,
					!Application.getPreferences().getCheckBetaUpdates());
		...
		private JsonArray retrieveAllReleaseObjects() throws UpdateCheckerException {
			...
			if (arr1 == null)
				return null;
			if (Communicator.UPDATE_URL_LATEST == null)
				return arr1;
		...
		public static JsonObject getLatestReleaseJSON(JsonArray jsonArr, String preTag, String[] tags,
				boolean onlyOfficial) throws UpdateCheckerException {
			if (jsonArr == null)
				return null;
			...
			for (int i = 0; i < jsonArr.size(); i++) {
				...
				// Init latestObj and latestName here so that only filtered objects and tags can
				// be assigned to them
		...
		public static ReleaseStatus compareLatest(String tag1, String tag2) throws UpdateCheckerException {
			...
			for (int i = 0; i < tag2Split.length; i++) {
				// If the loop is still going until this condition, you have the situation where
				// tag1 is e.g.
				...
				if (i >= tag1Split.length) {
					...
					// version (it's an official release)
					else {
				...
				try {
					...
					if (devTags.containsKey(tag1Split[i]) && devTags.containsKey(tag2Split[i])) {
						...
						// In case when e.g. tag1 is '20.alpha.01' and tag2 '20.alpha.02', go to the
						// next loop to compare '01' and '02'
						continue;
					}
		...
		private static void checkMalformedReleaseTag(String[] tagSplit) throws UpdateCheckerException {
			...
			for (int i = 0; i < tagSplit.length; i++) {
				try {
					...
					if (test < 0) {
						String message = String.format("Tag item must be greater than zero, tag: '%s'",
								String.join(".", tagSplit));
						log.warn(message);
						throw new UpdateCheckerException(message);
					}
				} catch (NumberFormatException e) {
					if (i == 0) {
						String message = String.format("First tag item must be decimal, tag: '%s'",
								String.join(".", tagSplit));
						log.warn(message);
						throw new UpdateCheckerException(message);
					}
