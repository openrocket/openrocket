 ...
 * by the Manager. This class uses the service provider mechanism described in
 * the
 * {@link java.util.ServiceLoader} class to enumerate all the
 * implementations of <code>ScriptEngineFactory</code>. <br>
 * <br>
 * The <code>ScriptEngineManager</code> provides a method to return a list of
 * all these factories
 * as well as utility methods which look up factories on the basis of language
 * name, file extension
 * and mime type.
 * <p>
 * The <code>Bindings</code> of key/value pairs, referred to as the "Global
 * Scope" maintained
 * by the manager is available to all instances of <code>ScriptEngine</code>
 * created
 * by the <code>ScriptEngineManager</code>. The values in the
 * <code>Bindings</code> are
...
public class ScriptEngineManagerRedux {
     ...
     * <code>ClassLoader</code> using the service provider mechanism.<br>
     * <br>
    ...
    /**
     * <code>getBindings</code> returns the value of the <code>globalScope</code>
     * field.
     ...
     * The algorithm first searches for a <code>ScriptEngineFactory</code> that has
     * been
     * registered as a handler for the specified name using the
     * <code>registerEngineName</code>
     * method.
     * <br>
     * <br>
     * If one is not found, it searches the set of <code>ScriptEngineFactory</code>
     * instances
     ...
     * 
     * @param shortName The short name of the <code>ScriptEngine</code>
     *                  implementation.
     *                  returned by the <code>getNames</code> method of its
     *                  <code>ScriptEngineFactory</code>.
     * @return A <code>ScriptEngine</code> created by the factory located in the
     *         search. Returns null
     *         if no such factory was found. The <code>ScriptEngineManager</code>
     *         sets its own <code>globalScope</code>
     *         <code>Bindings</code> as the <code>GLOBAL_SCOPE</code>
     *         <code>Bindings</code> of the newly
     ...
     * <code>ScriptEngineFactory</code> classes
     ...
     * 
     * @param name    The name to be associated with the
     *                <code>ScriptEngineFactory</code>.
    ...
    private void initEngines(final ClassLoader loader) {
        ...
        try {
            ServiceLoader<ScriptEngineFactory> loaders = ServiceLoader.load(ScriptEngineFactory.class, loader);
