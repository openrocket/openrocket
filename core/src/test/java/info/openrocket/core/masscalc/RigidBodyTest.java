...
public class RigidBodyTest extends BaseTestCase {
	...
	public void testTwoPointGeneral() {
		...
		y2 = MathUtil.pow2(delta13.getY());
		z2 = MathUtil.pow2(delta13.getZ());
		double I13xx = I1xx + m1 * (y2 + z2);
		double I13yy = I1t + m1 * (x2 + z2);
		double I13zz = I1t + m1 * (x2 + y2);

		x2 = MathUtil.pow2(delta23.getX());
		y2 = MathUtil.pow2(delta23.getY());
		z2 = MathUtil.pow2(delta23.getZ());
	...
	public void testRigidBodyCompoundCalculations() {
		...
		// these are a bit of a hack, and depend upon all the bodies being along the
		// y=0, z=0 line.
		double y4 = cm4_expected.getY();
		double I14ax = I1ax + m1 * MathUtil.pow2(Math.abs(body1.getCM().getY() - y4));
		double I24ax = I2ax + m2 * MathUtil.pow2(Math.abs(body2.getCM().getY() - y4));
		double I54ax = I5ax + m5 * MathUtil.pow2(Math.abs(body5.getCM().getY() - y4));

		double I14zz = I1t + m1 * MathUtil.pow2(Math.abs(body1.getCM().getY() - y4));
		double I24zz = I2t + m2 * MathUtil.pow2(Math.abs(body2.getCM().getY() - y4));
		double I54zz = I5t + m5 * MathUtil.pow2(Math.abs(body5.getCM().getY() - y4));
