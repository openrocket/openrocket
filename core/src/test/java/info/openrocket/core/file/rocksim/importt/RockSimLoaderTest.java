...
public class RockSimLoaderTest extends BaseTestCase {
    ...
    public void testFinsOnTransitions() throws IOException, RocketLoadException {
        ...
        Assert.assertEquals(" Transition 1 fore radius does not match", 0.0125,
                ((Transition) transition1).getForeRadius(), MathUtil.EPSILON);
        Assert.assertEquals(" Transition 1 aft radius does not match", 0.025, ((Transition) transition1).getAftRadius(),
                MathUtil.EPSILON);
        Assert.assertEquals(" Transition 1 shape does not match", Transition.Shape.CONICAL,
                ((Transition) transition1).getShapeType());

        Assert.assertEquals(" Transition 2 length does not match", 0.075, transition2.getLength(), MathUtil.EPSILON);
        Assert.assertEquals(" Transition 2 fore radius does not match", 0.025,
                ((Transition) transition2).getForeRadius(), MathUtil.EPSILON);
        Assert.assertEquals(" Transition 2 aft radius does not match", 0.0125,
                ((Transition) transition2).getAftRadius(), MathUtil.EPSILON);
        Assert.assertEquals(" Transition 2 shape does not match", Transition.Shape.CONICAL,
                ((Transition) transition2).getShapeType());
        ...
        Assert.assertEquals(" Fin set 1 fin tab length does not match", 0.05, freeformFinSet1.getTabLength(),
                MathUtil.EPSILON);
        Assert.assertEquals(" Fin set 1 fin tab height does not match", 0.0075, freeformFinSet1.getTabHeight(),
                MathUtil.EPSILON);
        Assert.assertEquals(" Fin set 1 fin tab offset does not match", 0.01, freeformFinSet1.getTabOffset(),
                MathUtil.EPSILON);
        ...
        Assert.assertEquals(" Fin set 2 fin tab length does not match", 0.03, freeformFinSet2.getTabLength(),
                MathUtil.EPSILON);
        Assert.assertEquals(" Fin set 2 fin tab height does not match", 0.005, freeformFinSet2.getTabHeight(),
                MathUtil.EPSILON);
        Assert.assertEquals(" Fin set 2 fin tab offset does not match", 0, freeformFinSet2.getTabOffset(),
                MathUtil.EPSILON);
    }
    ...
    public void testLoadFromStream() throws Exception {
        ...
        rocket = doc.getRocket();
        Assert.assertNotNull(rocket);
        Assert.assertEquals("Three Stage Everything Included Rocket - Override Total Mass/CG",
                doc.getRocket().getName());
        Assert.assertEquals(3, rocket.getStageCount());
    ...
    public void testSubAssemblyRocket() throws IOException, RocketLoadException {
        ...
        Assert.assertEquals(2, loader.getWarnings().size()); // can't add BodyTube to NoseCone, and can't add Transition
                                                             // to Transition
