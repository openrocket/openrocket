...
public class ExampleDesignDialog extends JDialog {
	...
	private final JList<ExampleDesignFile> designSelection;
	...
	private ExampleDesignDialog(ExampleDesignFile[] designs, Window parent) {
		...
		designSelection = new JList<ExampleDesignFile>(designs);
	...
	public static URL[] selectExampleDesigns(Window parent) {
		...
		List<ExampleDesignFile> selected = dialog.designSelection.getSelectedValuesList();
		URL[] urls = new URL[selected.size()];
		int i = 0;
		for (ExampleDesignFile file : selected) {
			urls[i++] = file.getURL();
		}
		return urls;
	}
