...
public class BodyTubeComponentTests extends BaseTestCase {
	...
	@Test
	public void testActingAsMotorMountCanBeSwitchedFromTrueToFales()
	{
		BodyTube bt = new BodyTube();
		assertFalse(bt.isMotorMount());

		bt.setMotorMount(true);
		assertTrue(bt.isMotorMount());

		bt.setMotorMount(false);
		assertFalse(bt.isMotorMount());
	}
}
