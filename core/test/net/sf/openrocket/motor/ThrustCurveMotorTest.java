...
public class ThrustCurveMotorTest {
	...
	private final ThrustCurveMotor motor = new ThrustCurveMotor.Builder()
			.setManufacturer(Manufacturer.getManufacturer("foo"))
			.setDesignation("X6")
			.setDescription("Description of X6")
			.setMotorType(Motor.Type.RELOAD)
			.setStandardDelays(new double[] { 0, 2, Motor.PLUGGED })
			.setDiameter(radius * 2)
			.setLength(length)
			.setTimePoints(new double[] { 0, 1, 3, 4 })
			.setThrustPoints(new double[] { 0, 2, 3, 0 })
			.setCGPoints(new Coordinate[] { new Coordinate(0.02, 0, 0, 0.05), new Coordinate(0.02, 0, 0, 0.05), new Coordinate(0.02, 0, 0, 0.05), new Coordinate(0.03, 0, 0, 0.03) })
			.setDigest("digestA")
			.build();
